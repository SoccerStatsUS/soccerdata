#!/usr/local/bin/env python
# -*- coding: utf-8 -*-

import datetime

l = [

    {
        'name': 'CD Broncos',
        'city': 'Choluteca, Honduras',
        },

    {
        'name': 'Inter Playa del Carmen',
        'city': 'Playa del Carmen, Mexico',
        },

    {
        'name': 'Maunabo Leones',
        'city': 'Maunabo, Puerto Rico',
        },

    { 'name': 'Omai Gold Seekers' }, # Guyana 


    # Jamaica

    {
        'name': 'Arnett Gardens',
        'founded': 1977,
        'city': 'Kingston, Jamaica',
        },

    {
        'name': 'August Town',
        'city': 'August Town, Jamaica',
        },

    {
        'name': 'Benfica (Jamaica)',
        'city': 'Drax Hall, Jamaica',
        },

    {
        'name': 'Boys\' Town',
        'city': 'Kingston, Jamaica',
        },

    {
        'name': 'Cavalier FC',
        'founded': datetime.datetime(1962, 8, 1),
        'city': 'Kingston, Jamaica',
        },

    {
        'name': 'Constant Spring',
        'city': 'Constant Spring, Jamaica',
        },

    {
        'name': 'Duhaney Park',
        'city': 'Duhaney Park, Jamaica',
        },

    {
        'name': 'Highgate United',
        'city': 'Portland, Jamaica',
        },

    {
        'name': 'Humble Lions',
        'founded': 1974,
        'city': 'May Pen, Jamaica',
        },

    {
        'name': 'Meadhaven United',
        'founded': 1977,
        'city': 'Kingston, Jamaica',
        },

    {
        'name': 'Naggo Head FC',
        'founded': 1979,
        'city': 'Portmore, Jamaica',
        },

    {
        'name': 'Montego Bay United',
        'founded': 1972,
        'city': 'Montego Bay, Jamaica',
        },

    {
        'name': 'Harbour View',
        'founded': 1974,
        'city': 'Kingston, Jamaica',
        },

    {
        'name': 'Santos (Jamaica)',
        'founded': 1964,
        'city': 'Kingston, Jamaica',
        },


    {
        'name': 'Portmore United',
        'city': 'Portmore, Jamaica',
        },

    {
        'name': 'Reno FC',
        'city': 'Savanna-la-Mar, Jamaica',
        },

    {
        'name': 'Rivoli United',
        'city': 'Spanish Town, Jamaica',
        },

    {
        'name': 'St. George\'s SC',
        'founded': 1962,
        'city': 'Lynch Park, Jamaica',
        },

    {
        'name': 'Savannah SC',
        'founded': 2007,
        'city': 'Savanna-la-Mar, Jamaica',
        },

    {
        'name': 'Sporting Central Academy',
        'founded': 2000,
        'city': 'Clarendon, Jamaica',
        },


    {
        'name': 'Tivoli Gardens',
        'founded': 1970,
        'city': 'Kingston, Jamaica',
        },

    {
        'name': 'Village United',
        'founded': 1968,
        'city': 'Falmouth, Jamaica',
        },

    {
        'name': 'Wadadah FC',
        'founded': 1983,
        'city': 'Montego Bay, Jamaica',
        },


    {
        'name': 'Waterhouse F.C.',
        'city': 'Kingston, Jamaica',
        'founded': 1968,
        },


    # Trinidad

    { 'name': 'Maple Club FC' }, # Trinidad

    {
        'name': 'North East Stars',
        'city': 'Sangre Grande, Trinidad and Tobago',
        },

    {
        'name': 'FC South End',
        'city': 'Marabella, Trinidad and Tobago',
        },

    {
        'name': 'Ma Pau SC',
        #'city': 'Marabella, Trinidad and Tobago',
        },

    {
        'name': 'South Starworld Strikers',
        'city': 'Couva, Trinidad and Tobago',
        },

    {
        'name': 'T&TEC',
        'city': 'San Fernando, Trinidad and Tobago',
        },

    {
        'name': 'St Ann\'s Rangers',
        'city': 'San Juan, Trinidad and Tobago',
        },

    {
        'name': 'Tobago United',
        'dissolved': 2011,
        'city': 'Tobago, Trinidad and Tobago',
        },

    {
        'name': 'Police FC',
        'founded': 1975,
        'city': 'Marabella, Trinidad and Tobago',
        },

    {
        'name': 'Arima Fire',
        'city': 'Malabar, Trinidad and Tobago',
        },


    {
        'name': 'Joe Public F.C.',
        'founded': 1996,
        'city': 'Tunapuna, Trinidad and Tobago',
        },

    {
        'name': 'San Juan Jabloteh',
        'founded': 1974,
        'dissolved': 2012,
        'city': 'Port of Spain, Trinidad and Tobago',
        },

    {
        'name': 'United Petrotrin',
        'founded': 1992, 
        'dissolved': 2009,
        'city': 'Palo Seco, Trinidad and Tobago',
        },

    {
        'name': 'W Connection',
        'founded': 1999,
        'city': 'Marabella, Trinidad and Tobago',
        },


    {
        'name': 'Defence Force',
        'founded': 1974,
        'city': 'Mucurapo, Trinidad and Tobago',
        },


    {
        'name': 'Caledonia AIA',
        'founded': 1979,
        'city': 'Morvant, Trinidad and Tobago',
        },



    # Barbados


    {
        'name': 'Notre Dame SC',
        'city': 'Bridgetown, Barbados',
        },

    {
        'name': 'Barbados Defence Force',
        'city': 'Bridgetown, Barbados',
        },

    {
        'name': 'Brittons Hill',
        'city': 'Bridgetown, Barbados',
        },

    {
        'name': 'Eden Stars',
        'city': 'Saint Michael, Barbados',
        },

    {
        'name': 'Pinelands United SC',
        'city': 'Pinelands, Barbados',
        },

    {
        'name': 'Pride of Gall Hill FC',
        'city': 'Christ Church, Barbados',
        },

    {
        'name': 'Silver Sands (Barbados)',
        'city': 'Christ Church, Barbados',
        },

    {
        'name': 'Technico (Barbados)',
        'city': 'Speightstown, Barbados',
        },

    {
        'name': 'Weymouth Wales',
        'city': 'Saint Philip, Barbados',
        },

    {
        'name': 'Youth Milan FC',
        'city': 'Saint Lucy, Barbados',
        },

    {
        'name': 'Paradise SC',
        'city': 'Dover, Barbados',
        },



    # Martinique


    {
        'name': 'Club Franciscain',
        'founded': 1936,
        'city': 'Le Francois, Martinique',
        },

    {
        'name': 'Gauloise de Trinité',
        'city': 'La Trinité, Martinique',
        },

    {
        'name': 'New Star',
        'city': 'Ducos, Martinique',
        },

    {
        'name': 'US Ducos Durivage',
        'city': 'Fort-de-France, Martinique',
        },


    {
        'name': 'Golden Lion FC',
        'city': 'Saint-Joseph, Martinique',
        },

    {
        'name': 'Eclair Rivière-Salée',
        'city': 'Rivière-Salée, Martinique',
        },

    {
        'name': 'Emulation (Schoelcher)',
        'city': 'Schoelcher, Martinique',
        },

    {
        'name': 'Golden Star',
        'founded': 1905,
        'city': 'Fort-de-France, Martinique',
        },

    {
        'name': 'Good Luck',
        'city': 'Fort-de-France, Martinique',
        },

    {
        'name': 'AS New Club',
        'founded': 1948,
        'city': 'Petit-Bourg, Martinique',
        },

    {
        'name': 'Réveil-Sportif',
        'founded': 1938,
        'city': 'Gros-Morne, Martinique',
        },

    {
        'name': 'Club Colonial',
        'founded': 1906,
        'city': 'Fort-de-France, Martinique',
        },

    {
        'name': 'Assaut de Saint-Pierre',
        'city': 'Saint-Pierre, Martinique',
        },

    {
        'name': 'CS Case-Pilote',
        'city': 'Case-Pilote, Martinique',
        },

    {
        'name': 'Gri-Gri',
        'city': 'Rivière-Pilote, Martinique',
        },

    {
        'name': 'US Marinoise',
        'city': 'Le Marin, Martinique',
        },

    {
        'name': 'RC Arlésien',
        'city': 'Les Anses-d\'Arlet, Martinique',
        },

    {
        'name': 'Etoile (Basse-Pointe)',
        'city': 'Basse-Pointe, Martinique',
        },

    {
        'name': 'Etoile (Rivière-Salée)',
        'city': 'Rivière-Salée, Martinique',
        },

    {
        'name': 'Olympique (Marin)',
        'city': 'Le Marin, Martinique',
        },

    {
        'name': 'Racing Club Saint-Joseph',
        'city': 'Saint-Joseph, Martinique',
        },


    {
        'name': 'RC Rivière-Pilote',
        'founded': 1961,
        'city': 'Rivière-Pilote, Martinique',
        },

    {
        'name': 'Santana FC',
        'city': 'Sainte-Anne, Martinique',
        },

    {
        'name': 'Club Péléen',
        'city': 'Morne Rouge, Martinique',
        },

    {
        'name': 'CS Vauclinois',
        'city': 'Le Vauclin, Martinique',
        },

    {
        'name': 'Racing Club Lorrain',
        'city': 'Le Lorrain, Martinique',
        },

    {
        'name': 'Anses d\'Arlet FC',
        'city': 'Les Anses-d\'Arlet, Martinique',
        },

    {
        'name': 'Silver Star',
        'city': 'L\'Ajoupa-Bouillon, Martinique',
        },

    {
        'name': 'AC Real Tartane',
        'city': 'La Trinité, Martinique',
        },

    {
        'name': 'CS Bélimois',
        'city': 'Le Lamentin, Martinique',
        },

    {
        'name': 'Samaritaine',
        'founded': 1920,
        'city': 'Sainte-Marie, Martinique',
        },

    {
        'name': 'US Diamantinoise',
        'city': 'Le Diamant, Martinique',
        },

    {
        'name': 'Stade Spiritain',
        'city': 'Saint-Esprit, Martinique',
        },


    {
        'name': 'Essor-Préchotain',
        'city': 'Le Prêcheur, Martinique',
        },

    {
        'name': 'AC Vert-Pré',
        'city': 'Vert-Pré, Martinique',
        },



    {
        'name': 'Aiglon du Lamentin',
        'founded': 1905,
        'city': 'Lamentin, Martinique',
        },


    {
        'name': 'US Robert',
        'founded': 1903,
        'city': 'Le Robert, Martinique',
        },

    # Saint Vincent and the Grenadines

    {
        'name': 'Stubborn Youth SC',
        'city': 'Saint Vincent and the Grenadines',
        },

    {
        'name': 'Camdonia Chelsea SC',
        'city': 'Saint Vincent and the Grenadines',
        },

    {
        'name': 'Fitz Hughes Predators',
        'city': 'Chateaubelair, Saint Vincent and the Grenadines',
        },


    {
        'name': 'System 3 FC',
        'founded': 1997,
        'city': 'Saint Vincent and the Grenadines',
        },

    {
        'name': 'Layou FC',
        'founded': 2010, # Founded in New York.
        'city': 'Layou, Saint Vincent and the Grenadines',
        },

    {
        'name': 'Zodiac FC',
        'founded': 1999,
        'city': 'Port Elizabeth, Saint Vincent and the Grenadines',
        },

    {
        'name': 'Prospect United',
        'founded': 1982,
        'city': 'Saint Vincent and the Grenadines',
        },


    {
        'name': 'Avenues United',
        'founded': 1974,
        'city': 'Kingstown, Saint Vincent and the Grenadines',
        },

    # US Virgin Islands

    {
        'name': 'New Vibes',
        'city': 'St. Thomas, US Virgin Islands',
        },



    {
        'name': 'Positive Vibes',
        'city': 'St. Thomas, US Virgin Islands',
        },


    {
        'name': 'Helenites',
        'city': 'Groveplace, US Virgin Islands',
        },


    # Curacao


    {
        'name': 'Centro Barber',
        'founded': 1951,
        'city': 'Willemstad, Curacao',
        },

    {
        'name': 'Inter Willemstad',
        'founded': datetime.datetime(1997, 4, 1),
        'city': 'Willemstad, Curacao',
        },

    {
        'name': 'S.V. S.U.B.T.',
        'founded': datetime.datetime(1925, 9, 25),
        'city': 'Willemstad, Curacao',
        },

    {
        'name': 'SV VESTA',
        'founded': datetime.datetime(1948, 6, 30),
        'city': 'Willemstad, Curacao',
        },


    {
        'name': 'Hubentut Fortuna',
        'founded': datetime.datetime(1994, 3, 1),
        'city': 'Willemstad, Curacao',
        },


    {
        'name': 'UNDEBA',
        'founded': datetime.datetime(1974, 5, 18),
        'city': 'Bándabou, Curacao',
        },


    {
        'name': 'SV Victory Boys',
        'founded': 1952,
        'city': 'Willemstad, Curacao',
        },


    {
        'name': 'Jong Holland',
        'founded': 1919,
        'city': 'Willemstad, Curacao',
        },

    {
        'name': 'Deportivo Santa Cruz',
        'founded': 1975,
        'city': 'Curacao',
        },

    {
        'name': 'RKSV Centro Dominguito',
        'founded': 1952,
        'city': 'Curacao',
        },


    {
        'name': 'Jong Colombia',
        'founded': datetime.datetime(1951, 7, 23),
        'city': 'Boca Sami, Curacao',
        },


    {
        'name': 'RKV FC SITHOC',
        'founded': datetime.datetime(1942, 3, 7),
        'city': 'Willemstad, Curacao',
        },

    # Guyana


    {
        'name': 'Milerock FC',
        'founded': 1972,
        'city': 'Linden, Guyana',
        },

    {
        'name': 'Guyana Defence Force',
        'founded': 1965,
        'city': 'Georgetown, Guyana',
        },

    {
        'name': 'Fruta Conquerors',
        'city': 'Tucville, Guyana',
        },

    {
        'name': 'Alpha United',
        'city': 'Providence, Guyana',
        },


    {
        'name': 'Bakewell Topp XX',
        'city': 'Liden, Guyana',
        },

    {
        'name': 'Buxton United',
        'city': 'East Demerara, Guyana',
        },

    {
        'name': 'Victoria Kings FC',
        'city': 'East Demerara, Guyana',
        },

    {
        'name': 'Camptown FC',
        'city': 'Georgetown, Guyana',
        },

    {
        'name': 'Rosignol United',
        'city': 'Berbice, Guyana',
        },

    {
        'name': 'Santos FC (Guyana)',
        'city': 'Georgetown, Guyana',
        },





    # Dominica

    {
        'name': 'Centre Bath Estate',
        'city': 'Roseau, Dominica',
        },

    # Saint Lucia

    {
        'name': 'Northern United All Stars',
        'city': 'Gros Islet, Saint Lucia',
        },



    {
        'name': 'Roots Alley Ballers',
        'city': 'Vieux Fort, Saint Lucia',
        },




    # Saint Kitts and Nevis



    {
        'name': 'Newtown United',
        'founded': 1962,
        'city': 'Basseterre, Saint Kitts and Nevis',
        },


    {
        'name': 'Garden Hotspurs FC',
        'city': 'Basseterre, Saint Kitts and Nevis',
        'founded': 1962,
        },



    # Cayman Islands



    {
        'name': 'Elite SC',
        'founded': 2006,
        'city': 'West Bay, Cayman Islands',
        },





    # Nicaragua

    {
        'name': 'América Managua',
        'city': 'Managua, Nicaragua',
        },

    {
        'name': 'Real Esteli',
        'founded': 1960,
        'city': 'Esteli, Nicaragua',
        },

    {
        'name': 'Atlético Estelí',
        'city': 'Esteli, Nicaragua',
        },


    {
        'name': 'Juventus Managua',
        'founded': 1977,
        'city': 'Managua, Nicaragua',
        },

    {
        'name': 'Flor de Caña',
        'city': 'Chichigalpa, Nicaragua',
        },

    {
        'name': 'Jalapa (Nicaragua)',
        'founded': 1995,
        'city': 'Jalapa, Nicaragua',
        },


    {
        'name': 'Diriangén FC',
        'founded': 1917,
        'city': 'Diriamba, Nicaragua',
        },

    {
        'name': 'Chinandega FC',
        'founded': 1975,
        'city': 'Chinandega, Nicaragua',
        },

    {
        'name': 'Managua F.C.',
        'founded': 2006,
        'city': 'Managua, Nicaragua',
        },


    {
        'name': 'Deportivo Ocotal',
        'founded': 2002,
        'city': 'Ocotal, Nicaragua',
        },

    {
        'name': 'Deportivo Bluefields',
        'city': 'Bluefields, Nicaragua',
        },

    {
        'name': 'Deportivo Masatepe',
        'city': 'Masatepe, Nicaragua',
        },

    {
        'name': 'Deportivo Masachapa',
        'city': 'Managua, Nicaragua',
        },

    {
        'name': 'Real Madriz',
        'founded': 1996,
        'city': 'Somoto, Nicaragua',
        },

    {
        'name': 'FC San Marcos',
        'city': 'San Marcos, Nicaragua',
        },

    {
        'name': 'Scorpión FC',
        'city': 'Chinandega, Nicaragua',
        },

    {
        'name': 'VCP Chinandega',
        'founded': 2006,
        'city': 'Chinandega, Nicaragua',
        },

    {
        'name': 'Deportivo Walter Ferretti',
        'founded': 1987,
        'city': 'Managua, Nicaragua',
        },

    {
        'name': 'Deportivo Santa Cecilia',
        'city': 'Diriamba, Nicaragua',
        },

    {
        'name': 'Xilotepelt',
        'city': 'Jinotepe, Nicaragua',
        },


    # Bermuda


    {
        'name': 'North Village Rams',
        'founded': 1957,
        'city': 'Bermuda',
        },

    {
        'name': 'Somerset Eagles',
        'city': 'Somerset, Bermuda',
        },


    {
        'name': 'Somerset Trojans',
        'city': 'Somerset, Bermuda',
        },

    {
        'name': 'Pembroke Hamilton Club',
        'city': 'Pembroke Parish, Bermuda',
        },


    {
        'name': 'Devonshire Cougars',
        'city': 'Devonshire, Bermuda',
        },

    {
        'name': 'Southampton Rangers',
        'city': 'Southampton, Bermuda',
        },

    {
        'name': 'Boulevard Blazers', # Bermuda
        },


    {
        'name': 'Dandy Town Hornets',
        'city': 'Pembroke Parish, Bermuda',
        },


    {
        'name': 'PHC Zebras',
        'city': 'Pembroke Parish, Bermuda',
        },



    {
        'name': 'Devonshire Colts FC',
        'founded': 1958,
        'city': 'Devonshire, Bermuda',
        },


    {
        'name': 'Bermuda Hogges',
        'founded': 2006,
        'city': 'Pembroke, Bermuda',
        },

    # Haiti


    {
        'name': 'Racing Club Haitien',
        'founded': 1923,
        'city': 'Port-au-Prince, Haiti',
        },

    {
        'name': 'Don Bosco',
        'city': 'Petionville, Haiti',
        },

    {
        'name': 'Racing des Gonaïves',
        'city': 'Gonaïves, Haiti',
        },

    {
        'name': 'Violette AC',
        'city': 'Port-au-Prince, Haiti',
        },


    {
        'name': 'Tempête Football Club',
        'founded': 1970,
        'city': 'Saint-Marc, Haiti',
        },


    {
        'name': 'AS Cavaly',
        'city': 'Léogâne, Haiti',
        },


    {
        'name': 'Roulado FC',
        'city': 'La Gonave, Haiti',
        },


    {
        'name': 'Aigle Noir',
        'city': 'Port-au-Prince, Haiti',
        },


    {
        'name': 'Baltimore Sportif',
        'founded': datetime.datetime(1974, 8, 1),
        'city': 'Saint-Marc, Haiti',
        },

    {
        'name': 'Tempête FC',
        'founded': 1970,
        'city': 'Saint-Marc, Haiti',
        },

    {
        'name': 'Victory SC',
        'founded': datetime.datetime(1947, 3, 7),
        'city': 'Port-au-Prince, Haiti',
        },


    {
        'name': 'Etoile Haïtienne',
        'city': 'Port-au-Prince, Haiti',
        },

    # Antigua and Barbuda


    {
        'name': 'SAP',
        'city': 'Bolans, Antigua and Barbuda',
        },


    {
        'name': 'Bassa',
        'city': 'All Saint\'s, Antigua and Barbuda',
        },

    {
        'name': 'Hoppers FC',
        'founded': 1969,
        'city': 'Green Bay, Antigua and Barbuda',
        },

    {
        'name': 'Antigua Barracuda FC',
        'founded': 2010,
        'city': 'Saint George Parish, Antigua and Barbuda',
        },

    # Suriname


    {
        'name': 'Inter Moengotapoe',
        'founded': 1992,
        'city': 'Moengo, Suriname',
        },

    {
        'name': 'SV Leo Victor',
        'founded': 1934,
        'city': 'Paramaribo, Suriname',
        },

    {
        'name': 'SV Robinhood',
        'founded': datetime.datetime(1945, 2, 6),
        'city': 'Paramaribo, Suriname',
        },

    {
        'name': 'SV Transvaal',
        'founded': datetime.datetime(1921, 1, 15),
        'city': 'Paramaribo, Suriname',
        },

    {
        'name': 'SV Voorwarts',
        'founded': datetime.datetime(1919, 8, 1),
        'city': 'Paramaribo, Suriname',
        },


    {
        'name': 'SV Voorwaarts',
        'founded': datetime.datetime(1919, 8, 1),
        'city': 'Paramaribo, Suriname',
        },


    {
        'name': 'FCS Nacional',
        'city': 'Paramaribo, Suriname',
        },


    {
        'name': 'Walking Bout Company',
        'founded': datetime.datetime(1997, 1, 16),
        'city': 'Paramaribo, Suriname',
        },


    # Belize

    {
        'name': 'Nizhee Corozal',
        'founded': 1985,
        'city': 'Corozal Town, Belize',
        },

    {
        'name': 'FC Belize',
        'founded': 2005,
        'city': 'Belize City, Belize',
        },

    {
        'name': 'Club Verdes',
        'founded': 1976,
        'city': 'Benque Viejo del Carmen, Belize',
        },

    {
        'name': 'Juventus (Belize)',
        'founded': 1978,
        'city': 'Orange Walk, Belize',
        },


    {
        'name': 'Revolutionary Conquerors',
        'founded': 2005,
        'city': 'Dangriga, Belize',
        },


    {
        'name': 'Kulture Yabra',
        'founded': 1974,
        'city': 'Belize City, Belize',
        },

    {
        'name': 'Placencia Pirates',
        'city': 'Placencia, Belize',
        },

    {
        'name': 'Wagiya',
        'city': 'Dangriga, Belize',
        'founded': 1984,
        },


    # Aruba


    {
        'name': 'SV Estrella',
        'founded': 1948,
        'city': 'Oranjestad, Aruba',
        },

    {
        'name': 'SV Racing Club Aruba',
        'founded': 1974,
        'city': 'Oranjestad, Aruba',
        },


    {
        'name': 'SV Deportivo Nacional',
        'city': 'Noord, Aruba',
        },


    {
        'name': 'SV River Plate Aruba',
        'city': 'Madiki, Aruba',
        'founded': datetime.datetime(1953, 2, 1),
        },


    {
        'name': 'SV Britannia',
        'founded': datetime.datetime(1958, 10, 12),
        'city': 'Piedra Plat, Aruba',
        },




    # Guadeloupe


    # Guadeloupe


    {
        'name': 'Zénith (Morne-à-l\'Eau)',
        'founded': 1958,
        'city': 'Morne-à-l\'Eau, Guadelopue',
        },

    {
        'name': 'La Gauloise de Basse-Terre',
        'city': 'Basse-Terre, Guadelopue',
        },





    {
        'name': 'CS Moulien',
        'founded': 1931,
        'city': 'Le Moule, Guadeloupe',
        },

    {
        'name': 'Solidarité Scolaire',
        'city': 'Baie-Mahault, Guadeloupe',
        },



    # French Guiana


    {
        'name': 'US Sinnamary',
        'city': 'Sinnamary, French Guiana',
        },


    # Mexico


    {
        'name': 'Chivas Tijuana',
        'founded': 1997,
        'dissolved': 1999, # became Nacional de Tijuana?
        'city': 'Tijuana, Mexico',
        },

    {
        'name': 'Cobras de Ciudad Juárez',
        'founded': 1980,
        'city': 'Ciudad Juarez, Mexico',
        },

    {
        'name': 'San Sebastián de León',
        'founded': 1944,
        'city': 'León, México',
        },

    {
        'name': 'C.F. La Piedad',
        'founded': datetime.datetime(1951, 11, 12),
        'city': 'La Piedad, Mexico',
        },

    {
        'name': 'Club Deportivo Zamora',
        'founded': datetime.datetime(1950, 12, 26),
        'city': 'Zamora de Hidalgo, Mexico',
        },

    {
        'name': 'Colibríes',
        'founded': datetime.datetime(2002, 12, 10),
        'city': 'Xochitepec, Morelos',
        },

    {
        'name': 'Germania FV',
        'founded': 1915,
        'city': 'Mexico City, Mexico',
        },

    {
        'name': 'Moctezuma de Orizaba',
        'founded': 1932,
        'dissolved': 1950,
        'city': 'Orizaba, Veracruz',
        },


    {
        'name': 'Cuautla FC',
        'founded': 1952,
        'city': 'Cuautla, Mexico',
        },

    {
        'name': 'Orizaba AC',
        'founded': 1898,
        'city': 'Orizaba, Mexico',
        },

    {
        'name': 'Ángeles de Puebla',
        'founded': 1984,
        'dissolved': 2001,
        'city': 'Puebla, Mexico',
        },

    {
        'name': 'Unión de Curtidores',
        'founded': datetime.datetime(1928, 8, 15),
        'city': 'Leon, Mexico',
        },


    {
        'name': 'Nacional de Guadalajara',
        'founded': 1916,
        'city': 'Guadalajara, Mexico',
        },

    {
        'name': 'Universidad de Guadalajara',
        'founded': 1970,
        'city': 'Guadalajara, Mexico',
        },

    {
        'name': 'CF Oaxtepec',
        'city': 'Oaxtepec, Mexico',
        },



    {
        'name': 'CD Zacatepec',
        'founded': 1948,
        'city': 'Zacatepeca, Mexico',
        },


    {
        'name': 'Deportivo Neza',
        'founded': 1978,
        'city': 'Ciudad Neza, Mexico',
        },

    {
        'name': 'Toros Neza',
        'founded': 1993, # re=founded 2011.
        'city': 'Nezahualcóyotl, Mexico',
        },

    {
        'name': 'Querétaro FC',
        'founded': 1950,
        'city': 'Querétaro, Mexico',
        },


    {
        'name': 'Cobras de Queretaro', # dissolved.
        'founded': 1985,
        'dissolved': 1994, # re-created in 2001 as youth team of Monterrey.
        'city': 'Queretaro, Mexico',
        },

    {
        'name': 'Club América',
        'founded': datetime.datetime(1916, 10, 12),
        'city': 'Mexico City, Mexico',
        },

    {
        'name': 'CF Asturias',
        'founded': datetime.datetime(1918, 2, 7),
        'city': 'Mexico City, Mexico',
        },

    {
        'name': 'Junior Club',
        'founded': 1906,
        'city': 'Mexico City, Mexico',
        },


    {
        'name': 'Atlético Celaya',
        'founded': 1994,
        'city': 'Celaya, Mexico',
        },

    {
        'name': 'Aurrerá',
        'founded': 1919,
        'city': 'Mexico City, Mexico',
        },

    {
        'name': 'Real Club España',
        'founded': datetime.datetime(1912, 3, 20),
        'city': 'Mexico City, Mexico',
        },

    {
        'name': 'Real Club España B',
        'city': 'Mexico City, Mexico',
        },

    {
        'name': 'Rovers FC Mexico',
        'founded': 1912,
        'city': 'Mexico City, Mexico',
        },

    {
        'name': 'Dorados de Sinaloa',
        'founded': datetime.datetime(2003, 8, 23),
        'city': 'Culiacan, Mexico',
        },


    {
        'name': 'Mexico Toros',
        'founded': 1995,
        'dissolved': 1995,
        'city': 'Mexico City, Mexico',
        },

    {
        'name': 'Monterrey Fury',
        'founded': 2003,
        'dissolved': 2005,
        'city': 'Monterrey, Mexico',
        },

    {
        'name': 'Monterrey La Raza',
        'founded': 1992,
        'dissolved': 2000,
        'city': 'Monterrey, Mexico',
        },



    {
        'name': 'Atlante F.C.',
        'founded': 1916,
        'city': 'Cancún, Mexico',
        },

    {
        'name': 'Atlas',
        'founded': datetime.datetime(1916, 8, 15),
        'city': 'Guadalajara, Mexico',
        },

    {
        'name': 'Atlético Español',
        'founded': datetime.datetime(1971, 9, 19),
        'dissolved': datetime.datetime(1982, 7, 21),
        'city': 'Mexico City, Mexico',
        },

    {
        'name': 'Atlético Potosino',
        'founded': 1956, 
        'city': 'San Luis Potosi, Mexico',
        },

    {
        'name': 'Celaya FC',
        'founded': datetime.datetime(1954, 2, 7),
        'city': 'Celaya, Mexico',
        },


    {
        'name': 'Cruz Azul',
        'founded': datetime.datetime(1927, 5, 22),
        'city': 'Mexico City, Mexico',
        },

    {
        'name': 'Real Sociedad de Zacatecas',
        'founded': 1996,
        'city': 'Zacatecas, Mexico',
        },

    {
        'name': 'Indios de Ciudad Juárez',
        'founded': 2005,
        'city': 'Ciudad Juarez, Mexico',
        },

    {
        'name': 'CD Irapuato',
        'founded': 1911, 
        'city': 'Irapuato, Mexico',
        },

    {
        'name': 'Jaguares de Chiapas',
        'founded': datetime.datetime(2002, 6, 27),
        'city': 'Tuxtla Gutiérrez, Mexico',
        },

    {
        'name': 'Club Jalisco',
        'founded': 1970,
        'city': 'Guadalajara, Mexico',
        },

    {
        'name': 'CD Guadalajara',
        'founded': datetime.datetime(1906, 5, 8),
        'city': 'Guadalajara, Mexico',
        },

    {
        'name': 'Club Leon',
        'city': 'León, Mexico',
        },

    {
        'name': 'Mexico Cricket Club',
        'founded': 1827,
        'city': 'Mexico City, Mexico',
        },

    {
        'name': 'Monarcas Morelia',
        'founded': datetime.datetime(1924, 11, 21),
        'city': 'Morelia, Mexico',
        },

    {
        'name': 'Tampico Madero FC',
        'founded': datetime.datetime(1945, 7, 8),
        'city': 'Tampaulipas, Mexico',
        },

    {
        'name': 'CF Monterrey',
        'founded': datetime.datetime(1945, 6, 28),
        'city': 'Monterrey, Mexico',
        },

    {
        'name': 'Necaxa',
        'founded': datetime.datetime(1923, 8, 21),
        'city': 'Aguascalientes, Mexico',
        },

    {
        'name': 'Nuevo León',
        'founded': 1957,
        'city': 'Monterrey, Mexico',
        },

    {
        'name': 'CD Oro',
        'founded': datetime.datetime(1923, 1, 5),
        'city': 'Guadalajara, Mexico',
        },

    {
        'name': 'C.F. Pachuca',
        'founded': 1901,
        'city': 'Pachuca, Mexico',
        },

    {
        'name': 'Puebla',
        'founded': datetime.datetime(1944, 5, 7),
        'city': 'Puebla, Mexico',
        },

    {
        'name': 'San Luis FC',
        'founded': datetime.datetime(1957, 9, 23),
        'city': 'San Luis Potosi, Mexico',
        },


    {
        'name': 'Santos Laguna',
        'founded': datetime.datetime(1983, 9, 4),
        'city': 'Torreon, Mexico',
        },

    {
        'name': 'Torreon FC',
        'founded': 1960,
        'city': 'Torreon, Mexico',
        },


    {
        'name': 'Laguna FC',
        'founded': 1953,
        'city': 'Torreon, Mexico',
        },


    {
        'name': 'L\'Amicale Francaise',
        'founded': 1911,
        'city': 'Mexico City, Mexico',
        },



    {
        'name': 'CD Tapatio',
        'founded': datetime.datetime(1973, 11, 2),
        'city': 'Guadalajara, Mexico',
        },

    {
        'name': 'Reforma AC',
        'founded': 1894,
        'city': 'Mexico City, Mexico',
        },

    {
        'name': 'British Club Mexico',
        'founded': 1902,
        'dissolved': 1912,
        'city': 'Mexico City, Mexico',
        },



    {
        'name': 'Tecos',
        'founded': 1935,
        'city': 'Guadalajara, Mexico',
        },

    {
        'name': 'Toluca',
        'founded': datetime.datetime(1917, 2, 12),
        'city': 'Toluca, Mexico',
        },

    {
        'name': 'UANL',
        'founded': datetime.datetime(1960, 3, 7),
        'city': 'Monterrey, Mexico',
        },

    {
        'name': 'UNAM',
        'founded': datetime.datetime(1954, 8, 28),
        'city': 'Mexico City, Mexico',
        },

    {
        'name': 'Tiburones Rojos de Veracruz',
        'founded': datetime.datetime(1943, 4, 9),
        'city': 'Veracruz, Mexico',
        },

    {
        'name': 'Correcaminos UAT',
        'founded': datetime.datetime(1980, 1, 1),
        'city': 'Ciudad Victoria, Mexico',
        },


     # Bonaire

    {
        'name': 'SV Juventus',
        'founded': 1973,
        'city': 'Kralendijk, Bonaire',
        },


    # Saint Lucia



    {
        'name': 'Rovers United FC',
        'city': 'Castries, Saint Lucia',
        },

    # Cayman Islands

    {
        'name': 'George Town SC',
        'founded': 1996,
        'city': 'George Town, Cayman Islands',
        },


    {
        'name': 'Bodden Town',
        'city': 'Bodden Town, Cayman Islands',
        },



    # Puerto Rico 



    {
        'name': 'River Plate Puerto Rico',
        'founded': 2007,
        'city': 'Fajardo, Puerto Rico',
        },

    {
        'name': 'Sevilla FC Puerto Rico',
        'founded': 2006,
        'city': 'Juncos, Puerto Rico',
        },

    {
        'name': 'Puerto Rico United',
        'founded': 2007,
        'city': 'Aguada, Puerto Rico',
        },






    {
        'name': 'Bayamón FC',
        'founded': 1999,
        'city': 'Bayamon, Puerto Rico',
        },



    {
        'name': 'Puerto Rico Islanders',
        'founded': 2003,
        'city': 'Bayamon, Puerto Rico',
        },

    # Dominica

    {
        'name': 'Sagicor South East United',
        'city': 'La Plaine, Dominica',
        },

    # Cuba


    {
        'name': 'Pinar del Río',
        'founded': 1978,
        'city': 'Pinar del Río, Cuba',
        },



    # Montserrat

    {
        'name': 'Ideal SC',
        'city': 'Brades, Montserrat',
        },





    

]
