#!/usr/local/bin/env python
# -*- coding: utf-8 -*-

import datetime

l = [


   
# Bosnia

    {
        'name': 'Željezničar Sarajevo',
        'city': 'Sarajevo, Bosnia',
        'founded': datetime.datetime(1921, 9, 19),
        },

    {
        'name': 'FK Sarajevo',
        'city': 'Sarajevo, Bosnia',
        'founded': datetime.datetime(1946, 10, 24),
        },


    # Albania

    {
        'name': 'Vllaznia Shkodër',
        'city': 'Shkodër, Albania',
        'founded': datetime.datetime(1919, 2, 16),
        },


    {
        'name': 'KF Elbasani',
        'city': 'Elbasan, Albania',
        'founded': 1913,
        },

    {
        'name': 'Flamurtari Vlorë',
        'city': 'Vlorë, Albania',
        'founded': datetime.datetime(1923, 3, 23),
        },


# Armenia

    {
        'name': 'Ararat Yerevan',
        'city': 'Yerevan, Armenia',
        'founded': datetime.datetime(1935, 5, 10),
        },

    {
        'name': 'Skeid',
        'city': 'Oslo, Norway',
        'founded': datetime.datetime(1915, 1, 1),
        },

    {
        'name': 'Sandnes Ulf',
        'city': 'Sandnes, Norway',
        'founded': datetime.datetime(1911, 6, 1),
        },

    {
        'name': 'DWS Amsterdam',
        'city': 'Amsterdam, Netherlands',
        'founded': datetime.datetime(1907, 10, 11),
        },



    {
        'name': 'SC Kriens',
        'city': 'Kriens, Switzerland',
        'founded': 1944,
        },

    {
        'name': 'SR Delémont',
        'city': 'Delémont, Switzerland',
        'founded': 1909,
        },


    # Macedonia

    {
        'name': 'FK Vardar',
        'city': 'Skopje, Macedonia',
        'founded': datetime.datetime(1947, 7, 22),
        },



    {
        'name': 'KF Tirana',
        'city': 'Tirana, Macedonia',
        'founded': datetime.datetime(1920, 8, 16),
        },

    {
        'name': 'Dinamo Tirana',
        'city': 'Tirana, Macedonia',
        'founded': datetime.datetime(1950, 3, 3),
        },



    {
        'name': 'Partizani Tirana',
        'city': 'Tirana, Macedonia',
        'founded': datetime.datetime(1946, 2, 4),
        },



    {
        'name': 'FC Haka',
        'city': 'Valkeakoski, Finland',
        'founded': 1934,
        },

    {
        'name': 'Helsingin Palloseura',
        'city': 'Helsinki, Finland',
        'founded': 1917,
        },

    {
        'name': 'Turun Palloseura',
        'city': 'Turku, Finland',
        'founded': 1922,
        },

    {
        'name': 'Oulun Palloseura',
        'city': 'Oulu, Finland',
        'founded': 1927,
        },
    {
        'name': 'FC Kuusysi',
        'city': 'Lahti, Finland',
        'founded': 1934,
        },


    {
        'name': 'HIFK Helsinki',
        'city': 'Helsinki, Finland',
        'founded': 1907,
        },

    {
        'name': 'KuPS',
        'city': 'Kuopio, Finland',
        'founded': 1923,
        },



    {
        'name': 'Lokomotiv Sofia',
        'city': 'Sofia, Bulgaria',
        'founded': datetime.datetime(1929, 9, 2),
        },

    {
        'name': 'Etar 1924',
        'city': 'Veliko Tarnovo, Bulgaria',
        'founded': 2002,
        'dissolved': datetime.datetime(2013, 5, 8),
        },

    {
        'name': 'Beroe',
        'city': 'Stara Zagora, Bulgaria',
        'founded': 1916,
        },

    {
        'name': 'Levski Sofia',
        'city': 'Sofia, Bulgaria',
        'founded': datetime.datetime(1914, 5, 24),
        },

    {
        'name': 'CSKA Sofia',
        'city': 'Sofia, Bulgaria',
        'founded': datetime.datetime(1948, 5, 5),
        },


    {
        'name': 'Sliema Wanderers',
        'city': 'Sliema, Malta',
        'founded': 1909,
        },

    {
        'name': 'Floriana',
        'city': 'Floriana, Malta',
        'founded': 1894,
        },

    {
        'name': 'Hibernians (Malta)',
        'city': 'Paola, Malta',
        'founded': 1922,
        },

    {
        'name': 'Csepel SC',
        'city': 'Budapest, Hungary',
        'founded': 1912,
        },

    {
        'name': 'Nagykanizsa',
        'city': 'Nagykanizsa, Hungary',
        },

    {
        'name': 'Spartak Plovdiv',
        'city': 'Plovdiv, Bulgaria',
        'founded': datetime.datetime(1947, 11, 15),
        },

    {
        'name': 'Botev Plovdiv',
        'city': 'Plovdiv, Bulgaria',
        'founded': datetime.datetime(1912, 3, 12),
        },

    {
        'name': 'UT Arad',
        'city': 'Arad, Romania',
        'founded': datetime.datetime(1945, 4, 18),
        },

    {
        'name': 'CF Brăila',
        'city': 'Brăila, Romania',
        'founded': 1922,
        },

    {
        'name': 'Concordia Chiajna',
        'city': 'Chiajna, Romania',
        'founded': 1957,
        },

    {
        'name': 'CS Otopeni',
        'city': 'Otopeni, Romania',
        'founded': 2001,
        'dissolved': 2013,
        },

    {
        'name': 'CS Turnu Severin',
        'city': 'Drobeta-Turnu Severin, Romania',
        'founded': 2007,
        'dissolved': 2013,
        },

    {
        'name': 'Corvinul Hunedoara',
        'city': 'Hunedoara, Romania',
        'founded': 1921,
        },

    {
        'name': 'FC Inter Sibiu',
        'city': 'Sibiu, Romania',
        'founded': 1982,
        'dissolved': 2000,
        },

    {
        'name': 'Sportul Studențesc',
        'city': 'Bucharest, Romania',
        'founded': datetime.datetime(1916, 2, 11),
        },

    {
        'name': 'Victoria Brăneşti',
        'city': 'Brăneşti, Romania',
        'founded': 1968,
        'dissolved': 2012,
        },

    {
        'name': 'Olimpia Satu Mare',
        'city': 'Satu Mare, Romania',
        'founded': 1921,
        },


    {
        'name': 'Gloria Bistriţa',
        'city': 'Bistrița, Romania',
        'founded': datetime.datetime(1922, 7, 6),
        },

    {
        'name': 'Flacăra Moreni',
        'city': 'Moreni, Romania',
        'founded': 1922,
        },

    {
        'name': 'Voinţa Sibiu',
        'city': 'Sibiu, Romania',
        'founded': 2007,
        'dissolved': 2012,
        },

    {
        'name': 'CFR Cluj',
        'city': 'Cluj-Napoca, Romania',
        'founded': 1907,
        },

    {
        'name': 'Universitatea Cluj',
        'city': 'Cluj-Napoca, Romania',
        'founded': 1919,
        },

    {
        'name': 'Gloria Buzău',
        'city': 'Buzău, Romania',
        'founded': datetime.datetime(1973, 6, 16),
        },

    {
        'name': 'CS Mioveni',
        'city': 'Mioveni, Romania',
        'founded': 2000,
        },

    {
        'name': 'FC Bihor Oradea',
        'city': 'Oradea, Romania',
        'founded': 1958,
        },

    {
        'name': 'Poli Timișoara',
        'city': 'Timișoara, Romania',
        'founded': datetime.datetime(1921, 12, 4),
        'dissolved': datetime.datetime(2012, 9, 3),
        },

    {
        'name': 'Jiul Petroșani',
        'city': 'Petroșani, Romania',
        'founded': 1919,
        },

    {
        'name': 'Internaţional Curtea de Argeş',
        'city': 'Curtea de Argeș, Romania',
        'founded': 2000,
        'dissolved': 2011,
        },

    {
        'name': 'Foresta Fălticeni',
        'city': 'Fălticeni, Romania',
        'founded': 1954,
        'dissolved': 2003,
        },

    {
        'name': 'UM Timișoara',
        'city': 'Timișoara, Romania',
        'founded': 1960,
        'dissolved': 2008,
        },

    {
        'name': 'Internațional Curtea de Argeș',
        'city': 'Curtea de Argeș, Romania',
        'founded': 2000,
        'dissolved': 2011,
        },

    {
        'name': 'Poli Iași',
        'city': 'Iași, Romania',
        'founded': datetime.datetime(1945, 4, 27),
        'dissolved': 2010,
        },



    {
        'name': 'FCM Bacău',
        'city': 'Bacău, Romania',
        'founded': 1950,
        },

    {
        'name': 'Astra Giurgiu',
        'city': 'Giurgiu, Romania',
        'founded': 1934,
        },


    {
        'name': 'Pandurii Târgu Jiu',
        'city': 'Târgu Jiu, Romania',
        'founded': 1962,
        },

    {
        'name': 'Universitatea Craiova',
        'city': 'Craiova, Romania',
        'founded': 1948,
        },

    {
        'name': 'KPV Kokkola',
        'city': 'Kokkola, Finland',
        'founded': 1930,
        },



    {
        'name': 'Valletta FC',
        'city': 'Valletta, Malta',
        'founded': 1943,
        },

    {
        'name': 'Reipas Lahti',
        'city': 'Lahti, Finland',
        },



    {
        'name': 'HJK',
        'city': 'Helsinki, Finland',
        'founded': datetime.datetime(1907, 6, 19),
        },

    {
        'name': 'Vojvodina Novi Sad',
        'city': 'Novi Sad, Serbia',
        'founded': datetime.datetime(1914, 3, 6),
        },

    {
        'name': 'Hvidovre IF',
        'city': 'Hvidovre, Denmark',
        'founded': datetime.datetime(1925, 10, 15),
        },

    {
        'name': 'Rapid București',
        'city': 'Bucharest, Romania',
        'founded': datetime.datetime(1923, 6, 25),
        },



    {
        'name': 'Petrolul Ploieşti',
        'city': 'Ploieşti, Romania',
        'founded': 1924,
        },

    {
        'name': 'Partick Thistle',
        'founded': 1876,
        'city': 'Glasgow, Scotland',
        },

    {
        'name': 'Gretna',
        'founded': 1946,
        'dissovled': 2008,
        'city': 'Gretna, Scotland',
        },

    {
        'name': 'Falkirk',
        'founded': 1876,
        'city': 'Falkirk, Scotland',
        },

    {
        'name': 'Livingston',
        'founded': 1943,
        'city': 'Livingston, Scotland',
        },

    {
        'name': 'Hamilton Academical',
        'founded': 1874,
        'city': 'Hamilton, Scotland',
        },

    {
        'name': 'Ross County',
        'founded': 1929,
        'city': 'Dingwall, Scotland',
        },

    {
        'name': 'Inverness CT',
        'founded': 1994,
        'city': 'Inverness, Scotland',
        },


    {
        'name': 'Third Lanark',
        'city': 'Glasgow, Scotland',
        'founded': 1872,
        'dissolved': 1967,
        },
    {
        'name': 'FC Barcelona B',
        'city': 'Barcelona, Spain',
        'founded': 1970,
        },

    {
        'name': 'Elche CF',
        'city': 'Elche, Spain',
        'founded': 1923,
        },

    {
        'name': 'Mérida',
        'city': 'Mérida, Spain',
        'founded': 1912,
        'dissolved': 2000,
        },

    {
        'name': 'Gimnàstic de Tarragona',
        'city': 'Tarragona, Spain',
        'founded': 1886,
        },

    {
        'name': 'Granada CF',
        'city': 'Granada, Spain',
        'founded': 1931,
        },

    {
        'name': 'CD Logroñés',
        'city': 'Logroño, Spain',
        'founded': 1940,
        },

    {
        'name': 'Rodos',
        'city': 'Rhodes, Greece',
        },

    {
        'name': 'Atlético Clube de Portugal',
        'city': 'Lisbon, Portugal',
        'founded': datetime.datetime(1942, 9, 18),
        },

    {
        'name': 'GD Chaves',
        'city': 'Chaves, Portugal',
        },

    {
        'name': 'Campomaiorense',
        'city': 'Campo Maior, Portugal',
        },

    # England
    {
        'name': 'Manchester United',
        'founded': 1878,
        'city': 'Manchester, England',
        },

    {
        'name': 'Millwall Lionesses',
        'founded': 1971,
        'city': 'Millwall, England',
        },

    {
        'name': 'Oldham Athletic',
        'founded': 1895,
        'city': 'Oldham, England',
        },

    {
        'name': 'Notts County',
        'founded': 1862,
        'city': 'Nottingham, England',
        },

    {
        'name': 'Milton Keynes',
        'founded': 2004,
        'city': 'Milton Keynes, England',
        },
    {
        'name': 'Cheltenham Town',
        'founded': 1887,
        'city': 'Cheltenham, England',
        },

    {
        'name': 'Preston North End',
        'founded': 1863,
        'city': 'Preston, England',
        },


    {
        'name': 'Exeter City',
        'founded': 1904,
        'city': 'Exeter, England',
        },

    {
        'name': 'Bradford City',
        'founded': 1903,
        'city': 'Bradford, England',
        },


    {
        'name': 'Corinthian F.C.',
        'founded': 1882,
        'dissolved': 1939,
        'city': 'London, England',
        'next': 'Corinthian-Casuals F.C.',
        },

    {
        'name': 'Birmingham City F.C.',
        'founded': 1875,
        'city': 'Birmingham, England',
        },

    {
        'name': 'Sheffield United',
        'founded': datetime.datetime(1889, 3, 22),
        'city': 'Sheffield, England',
        },

    {
        'name': 'Newcastle United',
        'founded': datetime.datetime(1892, 12, 9),
        'city': 'Newcastle, England',
        },

    {
        'name': 'Fulham F.C.',
        'founded': datetime.datetime(1879, 8, 16),
        'city': 'Fulham, England,'
        },

    {
        'name': 'Huddersfield Town',
        'founded': datetime.datetime(1908, 8, 15),
        'city': 'Huddersfield, England',
        },

    {
        'name': 'Ipswich Town F.C.',
        'founded': 1878,
        'city': 'Ipswich, England',
        },

    {
        'name': 'Blackburn Rovers',
        'founded': 1875,
        'city': 'Blackburn, England',
        },

    {
        'name': 'Burnley FC',
        'founded': 1882,
        'city': 'Burnley, England',
        },

    {
        'name': 'Everton',
        'founded': 1878,
        'city': 'Liverpool, England',
        },

    {
        'name': 'Fulham',
        'founded': datetime.datetime(1879, 8, 16),
        'city': 'London, England',
        },


    {
        'name': 'Norwich City F.C.',
        'founded': datetime.datetime(1902, 6, 17),
        'city': 'Norwich, England',
        },

    {
        'name': 'Walsall F.C.',
        'founded': 1888,
        'city': 'Walsall, England',
        },


    {
        'name': 'West Bromwich Albion',
        'founded': 1878,
        'city': 'West Bromwich, England',
        },


    {
        'name': 'West Ham United',
        'founded': 1895,
        'city': 'London, England',
        },



    {
        'name': 'Upton Park F.C.',
        'founded': 1866,
        'dissolved': 1911,
        'city': 'London, England',
        },


    {
        'name': 'Macclesfield Town F.C.',
        'founded': 1874,
        'city': 'Macclesfield, England',
        },

    {
        'name': 'Charlton Athletic',
        'founded': 1905,
        'city': 'London, England',
        },


    {
        'name': 'Hull City',
        'founded': 1904,
        'city': 'Kingston upon Hull, England',
        },

    {
        'name': 'Barnsley',
        'founded': 1887,
        'city': 'Barnsley, England',
        },

    
    {
        'name': 'Swansea City',
        'founded': 1912,
        'city': 'Swansea, England',
        },

    {
        'name': 'Reading',
        'founded': 1871,
        'city': 'Reading, England',
        },


    {
        'name': 'Bangor City F.C.',
        'founded': 1876,
        'city': 'Bangor, England',
        },

    {
        'name': 'Watford',
        'founded': 1881,
        'city': 'Hertfordshire, England',
        },

    {
        'name': 'Wigan Athletic F.C.',
        'founded': 1932,
        'city': 'Wigan, England',
        },

    {
        'name': 'Sunderland A.F.C.',
        'founded': 1879,
        'city': 'Sunderland, England',
        },

    {
        'name': 'Wycombe Wanderers',
        'founded': 1887,
        'city': 'High Wycombe, England',
        },
    {
        'name': 'Sheffield Wednesday',
        'founded': 1867,
        'city': 'Sheffield, England',
        },
    {
        'name': 'VfL Bochum',
        'founded': 1848,
        'city': 'Bochum, Germany',
        },

    {
        'name': 'Frankfurter FC Viktoria',
        'founded': datetime.datetime(1991, 2, 7),
        'dissolved': datetime.datetime(2012, 7, 1),
        'city': 'Frankfurt, Germany',
        },

    {
        'name': '1. FFC 08 Niederkirchen',
        'founded': datetime.datetime(2008, 4, 25),
        'city': 'Niederkirchen, Germany',
        },

    {
        'name': 'FFC Flaesheim-Hillen',
        'founded': 1999,
        'dissolved': 2001,
        'city': 'Haltern, Germany',
        },

    {
        'name': 'FFC Heike Rheine',
        'founded': datetime.datetime(1998, 3, 10),
        'city': 'Rheine, Germany',
        },

    {
        'name': 'Sportfreunde Siegen',
        'founded': datetime.datetime(1899, 7, 25),
        'city': 'Siegen, Germany',
        },

    {
        'name': 'Turbine Potsdam',
        'founded': datetime.datetime(1999, 4, 1),
        'city': 'Potsdam, Germany',
        },

    {
        'name': 'FFC Frankfurt',
        'founded': 1998,
        'city': 'Frankfurt, Germany',
        },

    {
        'name': 'Carl Zeiss Jena',
        'founded': datetime.datetime(1903, 5, 13),
        'city': 'Jena, Germany',
        },

    {
        'name': 'Erzgebirge Aue',
        'founded': datetime.datetime(1946, 3, 4),
        'city': 'Aue, Germany',
        },



    {
        'name': 'Dynamo Dresden',
        'city': 'Dresden, Germany',
        'founded': datetime.datetime(1953, 4, 12),
        },


    {
        'name': '1. FC Magdeburg',
        'founded': datetime.datetime(1965, 12, 22),
        'city': 'Magdeburg, Germany',
        },

    {
        'name': 'Southampton F.C.',
        'founded': 1979,
        'city': 'Southampton, England',
        },

    {
        'name': 'Bolton Wanderers',
        'founded': 1874,
        'city': 'Horwich, England',
        },


    {
        'name': 'Arsenal',
        'founded': 1886,
        'city': 'London, England',
        },

    {
        'name': 'Plymouth Argyle F.C.',
        'founded': 1886,
        'city': 'Plymouth, England',
        },


    {
        'name': 'Chelsea FC',
        'founded': datetime.datetime(1905, 3, 10),
        'city': 'London, England',
        },

    {
        'name': 'Coventry City FC',
        'founded': datetime.datetime(1883, 8, 13),
        'city': 'Coventry, England',
        },

    {
        'name': 'Stockport County F.C.',
        'founded': 1883,
        'city': 'Stockport, England',
        },


    {
        'name': 'Blackpool F.C.',
        'founded': datetime.datetime(1887, 7, 26),
        'city': 'Blackpool, England',
        },


    {
        'name': 'Workington A.F.C.',
        'founded': 1921,
        'city': 'Workington, England',
        },
    {
        'name': 'Portsmouth F.C.',
        'founded': 1898,
        'city': 'Portsmouth, England',
        },
    {
        'name': 'Port Vale F.C.',
        'founded': 1876,
        'city': 'Burslem, England',
        },

    {
        'name': 'Middlesbrough F.C.',
        'founded': 1876,
        'city': 'Middlesbrough, England',
        },
    {
        'name': 'Leicester City',
        'founded': 1884,
        'city': 'Leicester, England',
        },
    {
        'name': 'Shrewsbury Town',
        'founded': 1886,
        'city': 'New Meadow, England',
        },
    {
        'name': 'Derby County',
        'founded': datetime.datetime(1884, 2, 5),
        'city': 'Derby, England',
        },
    {
        'name': 'Lincoln City',
        'founded': 1884,
        'city': 'Lincoln, England',
        },
    {
        'name': 'Leeds United',
        'founded': 1919,
        'city': 'Leeds, England',
        },
    {
        'name': 'Manchester City F.C.',
        'founded': 1880,
        'city': 'Manchester City, England',
        },
    {
        'name': 'Nottingham Forest F.C.',
        'founded': 1865,
        'city': 'West Bridgford, England',
        },

    {
        'name': 'Brighton & Hove Albion',
        'founded': datetime.datetime(1901, 8, 21),
        'city': 'Brighton & Hove, England',
        },
    {
        'name': 'Luton Town F.C.',
        'founded': datetime.datetime(1885, 4, 11),
        'city': 'Luton, England',
        },

    {
        'name': 'Queens Park Rangers',
        'founded': 1882,
        'city': 'London, England',
        },

    {
        'name': 'Hereford United',
        'founded': 1924,
        'city': 'Hereford, England',
        },






    {
        'name': 'Athlone Town',
        'founded': 1887,
        'city': 'Athlone, England',
        },


    {
        'name': 'Worcester City F.C.',
        'founded': 1902,
        'city': 'Worcester, England',
        },
    {
        'name': 'Bristol City',
        'founded': 1897,
        'city': 'Bristol, England',
        },

    {
        'name': 'Doncaster Belles',
        'founded': 1969,
        'city': 'Doncaster, England',
        },

    {
        'name': 'Tranmere Rovers',
        'founded': 1884,
        'city': 'Tranmere, England',
        },

    {
        'name': 'Bristol Rovers',
        'founded': 1883,
        'city': 'Bristol, England',
        },

    {
        'name': 'Aston Villa F.C.',
        'founded': 1874,
        'city': 'Birminghamn, England',
        },

    {
        'name': 'Liverpool F.C.',
        'founded': datetime.datetime(1892, 3, 15),
        'city': 'Liverpool, England',
        },

    {
        'name': 'Tottenham Hotspur',
        'founded': 1882,
        'city': 'London, England',
        },

    {
        'name': 'Stoke City F.C.',
        'founded': 1863,
        'city': 'Stoke-on-Trent, England',
        },


    {
        'name': 'Wolverhampton Wanderers F.C.',
        'founded': 1877,
        'city': 'Wolverhampton, England',
        },

    {
        'name': 'Wisbech Town F.C.',
        'founded': 1920,
        'city': 'Wisbech, England',
        },


    {
        'name': 'Wimbledon FC',
        'founded': 1889,
        'city': 'Wimbledon, England',
        },

    {
        'name': 'King\'s Lynn F.C.',
        'founded': 1879,
        'dissolved': datetime.datetime(2009, 11, 25),
        'city': 'King\'s Lynn, England',
        },

    {
        'name': 'Ely City FC',
        'founded': 1885,
        'city': 'Ely, England',
        },

    {
        'name': 'Newmarket Town F.C.',
        'founded': datetime.datetime(1877, 11, 22),
        'city': 'Newmarket, England',
        },

    {
        'name': 'Peterborough United F.C.',
        'founded': 1934,
        'city': 'Peterborough, England',
        },

    {
        'name': 'Barking F.C.',
        'founded': 1880,
        'city': 'Barking, England',
        },

    {
        'name': 'Barry Town',
        'founded': 1912,
        'city': 'Barry, Wales',
        },

    {
        'name': 'Bromsgrove Rovers F.C.',
        'founded': 1885,
        'dissolved': 2010,
        'city': 'Bromsgrove, England',
        },
    {
        'name': 'Crystal Palace F.C.',
        'founded': datetime.datetime(1905, 9, 10),
        'city': 'South Norwood, England',
        },

    {
        'name': 'Pegasus A.F.C.',
        'founded': 1948,
        'city': 'Oxford, England',
        },

    {
        'name': 'Millwall F.C.',
        'founded': datetime.datetime(1885, 10, 3),
        'city': 'South Bermondsey, England',
        },

    {
        'name': 'Swindon Town',
        'founded': 1879,
        'city': 'Swindon, England',
        },



    # Germany

    {
        'name': 'TSV 1860 Rosenheim',
        'founded': 1860,
        'city': 'Rosenheim, Germany',
        },

    {
        'name': 'Chemie Leipzig',
        'founded': datetime.datetime(1990, 8, 1),
        'city': 'Leipzig, Germany',
        },

    {
        'name': 'FC Augsburg',
        'founded': datetime.datetime(1907, 8, 8),
        'city': 'Augsburg, Germany',
        },

    {
        'name': 'SV Darmstadt 98',
        'founded': datetime.datetime(1898, 5, 22),
        'city': 'Darmstadt, Germany',
        },

    {
        'name': 'Alemannia Aachen',
        'founded': datetime.datetime(1900, 12, 16),
        'city': 'Aachen, Germany',
        },

    {
        'name': 'SpVgg Greuther Fürth',
        'founded': datetime.datetime(1903, 9, 23),
        'city': 'Fürth, Germany',
        },

    {
        'name': 'KFC Uerdingen',
        'founded': 1905,
        'city': 'Krefeld, Germany',
        },

    {
        'name': 'TSV Malente',
        'city': 'Malente, Germany',
        },

    {
        'name': 'TuS Mayen',
        'founded': 1886,
        'city': 'Mayene, Germany',
        },


    {
        'name': 'Eintracht Frankfurt',
        'founded': datetime.datetime(1899, 3, 8),
        'city': 'Frankfurt,Germany',
        },

    {
        'name': 'FSV Frankfurt',
        'founded': datetime.datetime(1899, 8, 20),
        'city': 'Frankfurt,Germany',
        },

    {
        'name': '1. FC Nuremberg',
        'founded': datetime.datetime(1900, 5, 4),
        'city': 'Nuremberg, Germany',
        },

    {
        'name': 'Itzehoer SV',
        'founded': datetime.datetime(1909, 10, 3),
        'city': 'Itzehoe, Germany',
        },

    {
        'name': 'Stuttgart Kickers',
        'founded': datetime.datetime(1899, 9, 21),
        'city': 'Stuttgart, Germany',
        },

    {
        'name': '1. FC Kaiserslautern',
        'founded': datetime.datetime(1900, 6, 2),
        'city': 'Kaiserslautern, Germany',
        },

    {
        'name': 'Mainz 05',
        'founded': datetime.datetime(1905, 3, 16),
        'city': 'Mainz, Germany',
        },

    {
        'name': '1. FC Nürnberg',
        'founded': datetime.datetime(1900, 5, 4),
        'city': 'Nuremberg, Germany',
        },

    # Same team?????
    {
        'name': 'Karlsruher SC',
        'founded': datetime.datetime(1894, 6, 6),
        'city': 'Karlsruhe, Germany',
        },
    {
        'name': 'Karlsruher FV',
        'founded': 1891,
        'city': 'Karlsruhe, Germany',
        },




    {
        'name': 'SSV Reutlingen 05',
        'founded': datetime.datetime(1905, 5, 9),
        'city': 'Reutlingen, Germany',
        },



    {
        'name': 'Preussen Munster',
        'founded': datetime.datetime(1906, 4, 30),
        'city': 'Munster, Germany',
        },

    {
        'name': 'Fortuna Düsseldorf',
        'founded': datetime.datetime(1895, 5, 5),
        'city': 'Dusseldorf, Germany',
        },

    {
        'name': 'Werder Bremen',
        'founded': datetime.datetime(1899, 2, 4),
        'city': 'Bremen, Germany',
        },


    {
        'name': 'VfL Wolfsburg',
        'founded': datetime.datetime(1945, 9, 12),
        'city': 'Wolfsburg, Germany',
        },


    {
        'name': 'Bayer Leverkusen',
        'founded': datetime.datetime(1904, 7, 1),
        'city': 'Leverkusen, Germany',
        },

    {
        'name': 'Bayern Munich',
        'founded': datetime.datetime(1900, 2, 27),
        'city': 'Munich, Germany',
        },

    {
        'name': '1. FC Saarbrucken',
        'founded': 1903,
        'city': 'Saarbrucken, Germany',
        },

    {
        'name': 'Eintracht Braunschweig',
        'founded': datetime.datetime(1895, 12, 15),
        'city': 'Braunschweig, Germany',
        },

    {
        'name': 'FFC Brauweiler',
        'founded': datetime.datetime(2000, 6, 1),
        'city': 'Pulheim, Germany',
        },

    {
        'name': 'SC 07 Bad Neuenahr',
        'founded': 1907,
        'city': 'Bad Neuenahr-Ahrweiler, Germany',
        },

    {
        'name': '1860 Munich',
        'founded': datetime.datetime(1860, 5, 17),
        'city': 'Munich, Germany',
        },


    {
        'name': 'SG Egelsbach',
        'founded': datetime.datetime(1945, 11, 10),
        'city': 'Egelsbach, Germany',
        },

    {
        'name': 'FC St. Pauli',
        'founded': datetime.datetime(1910, 5, 15),
        'city': 'St.Pauli, Germany',
        },

    {
        'name': 'Hannover 96',
        'founded': datetime.datetime(1896, 4, 12),
        'city': 'Hannover, Germany',
        },


    {
        'name': 'FK Pirmasens',
        'founded': 1903,
        'city': 'Pirmasens, Germany',
        },


    
    {
        'name': 'Hertha BSC',
        'founded': datetime.datetime(1892, 7, 25),
        'city': 'Berlin, Germany',
        },

    {
        'name': 'Dynamo Berlin',
        'founded': 1953,
        'city': 'Berlin, Germany',
        },

    {
        'name': 'SpVgg Unterhaching',
        'founded': datetime.datetime(1925, 1, 1),
        'city': 'Unterhaching, Germany',
        },

    {
        'name': 'MSV Duisburg',
        'founded': 1902,
        'city': 'Duisburg, Germany',
        },

    {
        'name': 'FCR 2001 Duisburg',
        'founded': datetime.datetime(2001, 6, 8),
        'city': 'Duisburg, Germany',
        },

    {
        'name': 'FF USV Jena',
        'founded': 2004,
        'city': 'Jena, Germany',
        },

    {
        'name': 'VfL Sindelfingen',
        'founded': 1862,
        'city': 'Sindelfingen, Germany',
        },

    {
        'name': 'SG Wattenscheid 09',
        'founded': 1909,
        'city': 'Wattenscheid, Germany',
        },

    {
        'name': 'Herforder SV',
        'founded': 1953,
        'city': 'Herford, Germany',
        },

    {
        'name': 'Hansa Rostock',
        'founded': datetime.datetime(1965, 12, 28),
        'city': 'Rostock, Germany',
        },


    {
        'name': 'Hertha Berlin',
        'founded': datetime.datetime(1892, 7, 25),
        'city': 'Berlin, Germany',
        },

    {
        'name': '1899 Hoffenheim',
        'founded': datetime.datetime(1899, 7, 1),
        'city': 'Hoffenheim, Germany',
        },
    {
        'name': 'Tennis Borussia Berlin',
        'founded': datetime.datetime(1902, 4, 9),
        'city': 'Berlin, Germany',
        },

    {
        'name': 'VfB Stuttgart',
        'founded': datetime.datetime(1912, 4, 1),
        'city': 'Stuttgart, Germany',
        },

    {
        'name': 'SC Freiburg',
        'founded': datetime.datetime(1904, 5, 30),
        'city': 'Freiburg im Breisgau, Germany',
        },





    {
        'name': 'VfB Oldenburg',
        'founded': 1897,
        'city': 'Oldenburg, Germany',
        },

    {
        'name': 'SSV Ulm 1846',
        'founded': 1846,
        'city': 'Ulm, Germany',
        },

    {
        'name': 'Borussia Dortmund',
        'founded': datetime.datetime(1909, 12, 19),
        'city': 'Dortmund, Germany',
        },
    {
        'name': 'Borussia Mönchengladbach',
        'founded': datetime.datetime(1900, 8, 1),
        'city': 'Mönchengladbach, Germany',
        },

    {
        'name': 'Rot-Weiss Essen',
        'founded': datetime.datetime(1907, 2, 1),
        'city': 'Essen, Germany',
        },

    {
        'name': 'SGS Essen',
        'founded': 2000,
        'city': 'Essen, Germany',
        },

    {
        'name': 'TSV Crailsheim',
        'founded': 1864,
        'city': 'Crailsheim, Germany',
        },

    {
        'name': 'Hamburger SV',
        'founded': datetime.datetime(1887, 9, 29),
        'city': 'Hamburg, Germany',
        },
    {
        'name': '1. FC Köln',
        'founded': datetime.datetime(1948, 2, 13),
        'city': 'Cologne, Germany',
        },


    {
        'name': 'FC Schalke 04',
        'founded': datetime.datetime(1904, 5, 4),
        'city': 'Gelsenkirchen, Germany',
        },


    {
        'name': 'SV Munster',
        'founded': datetime.datetime(1946, 5, 5),
        'city': 'Munster, Germany',
        },
    {
        'name': 'VfR Heilbronn',
        'founded': 1896,
        'dissolved': 2003,
        'city': 'Heilbronn, Germany',
        },
    {
        'name': 'VfR Pforzheim',
        'founded': 1897,
        'dissolved': datetime.datetime(2010, 6, 30),
        'city': 'Pforzheim, Germany',
        },
    {
        'name': 'VfL Osnabrück',
        'founded': 1899,
        'city': 'Osnabruck, Germany',
        },
    {
        'name': 'SpVgg Plattling',
        'founded': 1919,
        'city': 'Plattling, Germany',
    
        },


    {
        'name': 'TSV Ottobeuren',
        'city': 'Ottobeuren, Germany',
        },


    {
        'name': 'ASV Bergedorf',
        'founded': datetime.datetime(1885, 3, 29),
        'city': 'Bergedorf, Germany',
        },
    {
        'name': 'VfL Marburg',
        'founded': datetime.datetime(1860, 7, 28),
        'city': 'Marburg, Germany',
        },


    {
        'name': 'SpVgg Kaufbeuren',
        'founded': 1909,
        'city': 'Kaufbeuren, Germany',
        },
    {
        'name': 'Energie Cottbus',
        'founded': datetime.datetime(1966, 1, 31),
        'city': 'Cottbus, Germany',
        },

    {
        'name': 'Arminia Bielefeld',
        'founded': datetime.datetime(1905, 5, 3),
        'city': 'Bielefeld, Germany',
        },
    {
        'name': 'VfL Oldesloe',
        'founded': 1862,
        'city': 'Bald Oldesloe, Germany',
        },



    # Slovakia

    {
        'name': 'Spartak Trnava',
        'founded': datetime.datetime(1923, 5, 30),
        'city': 'Trnava, Slovakia',
        },

    {
        'name': 'FK Inter Bratislava',
        'founded': datetime.datetime(1940, 7, 1),
        'city': 'Bratislava, Slovakia',
        },

    {
        'name': 'ŠK Slovan Bratislava',
        'founded': datetime.datetime(1919, 5, 3),
        'city': 'Bratislava, Slovakia',
        },


    # Wales

    {
        'name': 'Cardiff City',
        'founded': 1889,
        'city': 'Cardiff, Wales',
        },

    # Sweden

    {
        'name': 'AIK Fotboll',
        'founded': datetime.datetime(1891, 2, 15),
        'city': 'Stockholm, Sweden',
        },

    {
        'name': 'BK Kenty',
        'founded': 1932,
        'city': 'Linkoping, Sweden',
        },

    {
        'name': 'Umeå IK',
        'founded': 1917,
        'city': 'Umeå, Sweden',
        },

    {
        'name': 'Bälinge IF',
        'founded': 1936,
        'city': 'Uppsala, Sweden',
        },

    {
        'name': 'Sunnanå SK',
        'founded': 1939,
        'city': 'Skellefteå, Sweden',
        },

    {
        'name': 'Holmalunds IF',
        'founded': 1933,
        'city': 'Skellefteå, Sweden',
        },

    {
        'name': 'Mallbackens IF',
        'founded': 1942,
        'city': 'Värmland, Sweden',
        },

    {
        'name': 'Västerås',
        'founded': 1904,
        'city': 'Västerås, Sweden',
        },
    {
        'name': 'Västra Frölunda',
        'founded': 1930,
        'city': 'Gothenburg, Sweden',
        },


    {
        'name': 'Syrianska FC',
        'founded': datetime.datetime(1977, 7, 1),
        'city': 'Södertälje, Sweden',
        },

    {
        'name': 'Degerfors',
        'founded': 1907,
        'city': 'Degerfors, Sweden',
        },

    {
        'name': 'GIF Sundsvall',
        'founded': datetime.datetime(1903, 8, 25),
        'city': 'Sundsvall, Sweden',
        },


    {
        'name': 'IK Brage',
        'founded': 1925,
        'city': 'Borlange, Sweden',
        },


    {
        'name': 'IFK Norrkoping',
        'founded': datetime.datetime(1897, 5, 29),
        'city': 'Norrkoping, Sweden',
        },

    {
        'name': 'Mjällby',
        'founded': datetime.datetime(1939, 4, 1),
        'city': 'Hällevik, Sweden',
        },

    {
        'name': 'Trelleborg',
        'founded': 1926,
        'city': 'Trelleborg, Sweden',
        },

    {
        'name': 'Helsingborgs IF',
        'founded': datetime.datetime(1907, 6, 4),
        'city': 'Helsingborg, Sweden',
        },

    {
        'name': 'BK Häcken',
        'founded': datetime.datetime(1940, 8, 2),
        'city': 'Gothenburg, Sweden',
        },

    {
        'name': 'Assyriska',
        'founded': 1974,
        'city': 'Södertälje, Sweden',
        },

    {
        'name': 'Åtvidaberg',
        'founded': datetime.datetime(1907, 7, 1),
        'city': 'Åtvidaberg, Sweden',
        },


    {
        'name': 'Örebro SK',
        'founded': datetime.datetime(1908, 10, 28),
        'city': 'Orebro, Sweden',
        },
    {
        'name': 'Brommapojkarna',
        'founded': 1942,
        'city': 'Stockholm, Sweden',
        },

    {
        'name': 'Djurgårdens IF',
        'founded': datetime.datetime(1891, 3, 12),
        'city': 'Stockholm, Sweden',
        },

    {
        'name': 'Örgryte IS',
        'founded': 1887,
        'city': 'Gothenburg, Sweden',
        },

    {
        'name': 'Hammarby IF',
        'founded': datetime.datetime(1897, 4, 10),
        'city': 'Johanneshov, Sweden',
        },

    {
        'name': 'Gefle IF',
        'founded': datetime.datetime(1882, 12, 5),
        'city': 'Gavle, Sweden',
        },

    {
        'name': 'Enköpings SK',
        'founded': 1914,
        'city': 'Enköping, Sweden',
        },

    {
        'name': 'GAIS',
        'founded': datetime.datetime(1894, 3, 11),
        'city': 'Gothenburg, Sweden',
        },

    {
        'name': 'Malmo FF',
        'founded': 1910,
        'city': 'Malmo, Sweden',
        },

    {
        'name': 'Landskrona BoIS',
        'founded': 1915,
        'city': 'Landskrona, Sweden',
        },

    {
        'name': 'IFK Göteborg',
        'founded': datetime.datetime(1904, 10, 4),
        'city': 'Gothenburg, Sweden',
        },


    {
        'name': 'Kalmar FF',
        'founded': datetime.datetime(1910, 1, 10),
        'city': 'Kalmar, Sweden',
        },

    {
        'name': 'IF Elfsborg',
        'founded': datetime.datetime(1904, 6, 26),
        'city': 'Elfsborg, Sweden',
        },

    {
        'name': 'Öster',
        'founded': 1930,
        'city': 'Vaxjo, Sweden',
        },

    {
        'name': 'Ljungskile SK',
        'founded': 1926,
        'city': 'Ljungskile, Sweden',
        },

    {
        'name': 'Kristianstads DFF',
        'founded': 1998,
        'city': 'Kristianstad, Sweden',
        },

    {
        'name': 'Alviks IK',
        'city': 'Alvik, Sweden',
        },


    {
        'name': 'QBIK',
        'founded': 1978,
        'city': 'Karlstad, Sweden',
        },

    {
        'name': 'IF Trion',
        'founded': 1936,
        'city': 'Spjutsbygd, Sweden',
        },


    # Norway

    {
        'name': 'Viking FK',
        'founded': datetime.datetime(1899, 8, 10),
        'city': 'Stavanger, Norway',
        },

    {
        'name': 'Bryne',
        'founded': datetime.datetime(1926, 4, 10),
        'city': 'Bryne, Norway',
        },

    {
        'name': 'Sogndal',
        'founded': datetime.datetime(1926, 2, 19),
        'city': 'Sogndal, Norway',
        },

    {
        'name': 'Rosenborg',
        'founded': datetime.datetime(1917, 5, 19),
        'city': 'Trondheim, Norway',
        },

    {
        'name': 'Strømsgodset',
        'founded': datetime.datetime(1907, 2, 10),
        'city': 'Drammen, Norway',
        },

    {
        'name': 'Hønefoss',
        'founded': datetime.datetime(1895, 2, 4),
        'city': 'Honefoss, Norway',
        },

    {
        'name': 'Kongsvinger',
        'founded': 1892,
        'city': 'Kongsvinger, Norway',
        },

    {
        'name': 'IK Start',
        'founded': datetime.datetime(1905, 9, 19),
        'city': 'Kristiansandd, Norway',
        },

    {
        'name': 'Hamarkameratene',
        'founded': datetime.datetime(1918, 8, 10),
        'city': 'Hamar, Norway',
        },


    {
        'name': 'Lillestrøm',
        'founded': datetime.datetime(1917, 4, 2),
        'city': 'Lillestrom, Norway',
        },

    {
        'name': 'Bodø/Glimt',
        'founded': datetime.datetime(1916, 9, 19),
        'city': 'Bodo, Norway',
        },

    {
        'name': 'Tromsø IL',
        'founded': datetime.datetime(1920, 9, 15),
        'city': 'Tromso, Norway',
        },

    {
        'name': 'Fredrikstad',
        'founded': datetime.datetime(1903, 4, 7),
        'city': 'Fredrikstad, Norway',
        },


    {
        'name': 'Sandefjord',
        'founded': datetime.datetime(1998, 9, 10),
        'city': 'Sandefjord, Norway',
        },

    {
        'name': 'Stavanger IF',
        'founded': datetime.datetime(1905, 9, 17),
        'city': 'Stavanger, Norway',
        },

    {
        'name': 'Sarpsborg FK',
        'founded': datetime.datetime(1903, 5, 8),
        'city': 'Sarpsborg, Norway',
        },

    {
        'name': 'Moss FK',
        'founded': datetime.datetime(1906, 8, 28),
        'city': 'Moss, Norway',
        },

    {
        'name': 'Larvik Turn',
        'founded': 1865,
        'city': 'Larvik, Norway',
        },
    {
        'name': 'Odds BK',
        'founded': datetime.datetime(1894, 3, 31),
        'city': 'Skien, Norway',
        },

    {
        'name': 'SK Brann',
        'founded': datetime.datetime(1908, 9, 26),
        'city': 'Bergen, Norway',
        },

    {
        'name': 'SK Djerv 1919',
        'founded': 1919,
        'city': 'Haugesund, Norway',
        },

    {
        'name': 'Molde',
        'founded': datetime.datetime(1911, 6, 19),
        'city': 'Molde, Norway',
        },

    {
        'name': 'Stabæk',
        'founded': datetime.datetime(1912, 3, 16),
        'city': 'Bærum, Norway',
        },

    {
        'name': 'Halmstads BK',
        'founded': 1914,
        'city': 'Orjans Vall, Norway',
        },

    {
        'name': 'SK Vard Haugesund',
        'founded': 1916,
        'city': 'Haugesund, Norway',
        },

    {
        'name': 'Haugesund',
        'founded': datetime.datetime(1993, 10, 28),
        'city': 'Haugesund, Norway',
        },



    {
        'name': 'Lyn Oslo',
        'founded': datetime.datetime(1896, 3, 3),
        'city': 'Oslo, Norway',
        },

    {
        'name': 'Aalesund',
        'founded': datetime.datetime(1914, 6, 25),
        'city': 'Alesund, Norway',
        },

    {
        'name': 'Vålerenga',
        'founded': datetime.datetime(1913, 7, 29),
        'city': 'Oslo, Norway',
        },


    {
        'name': 'Fredrikstad FK',
        'founded': datetime.datetime(1903, 4, 7),
        'city': 'Fredrikstad, Norway',
        },

    {
        'name': 'Mjøndalen IF',
        'founded': datetime.datetime(1910, 8, 22),
        'city': 'Mjøndalen, Norway',
        },



    # Netherlands

    {
        'name': 'FC Volendam',
        'founded': datetime.datetime(1977, 6, 1),
        'city': 'Volendam, Netherlands',
        },

    {
        'name': 'DWS',
        'founded': datetime.datetime(1907, 10, 11),
        'city': 'Amsterdam, Netherlands',
        },

    {
        'name': 'RKC Waalwijk',
        'founded': datetime.datetime(1940, 8, 26),
        'city': 'Waalwijk, Netherlands',
        },

    {
        'name': 'Heracles Almelo',
        'founded': datetime.datetime(1903, 5, 3),
        'city': 'Almelo, Netherlands',
        },

    {
        'name': 'PEC Zwolle',
        'founded': datetime.datetime(1910, 6, 12),
        'city': 'Zwolle, Netherlands',
        },

    {
        'name': 'FC Twente',
        'founded': datetime.datetime(1965, 7, 1),
        'city': 'Enschede, Netherlands',
        },

    {
        'name': 'Heerenveen',
        'founded': datetime.datetime(1920, 7, 20),
        'city': 'Heerenveen, Netherlands',
        },

    {
        'name': 'SC Cambuur',
        'founded': datetime.datetime(1946, 6, 19),
        'city': 'Leeuwarden, Netherlands',
        },

    {
        'name': 'Vitesse',
        'founded': datetime.datetime(1892, 5, 14),
        'city': 'Arnhem, Netherlands',
        },

    {
        'name': 'NEC',
        'founded': datetime.datetime(1900, 11, 15),
        'city': 'Nijmegen, Netherlands',
        },

    {
        'name': 'Excelsior',
        'founded': datetime.datetime(1902, 7, 23),
        'city': 'Rotterdam, Netherlands',
        },

    {
        'name': 'De Graafschap',
        'founded': datetime.datetime(1954, 2, 1),
        'city': 'Doetinchem, Netherlands',
        },

    {
        'name': 'AZ',
        'founded': datetime.datetime(1967, 5, 10),
        'city': 'Alkmaar, Netherlands',
        },

    {
        'name': 'AFC Ajax',
        'founded': datetime.datetime(1900, 3, 18),
        'city': 'Amsterdam, Netherlands',
        },

    {
        'name': 'PSV Eindhoven',
        'founded': datetime.datetime(1913, 8, 31),
        'city': 'Eindhoven, Netherlands',
        },

    {
        'name': 'FC Eindhoven',
        'founded': datetime.datetime(1909, 11, 16),
        'city': 'Eindhoven, Netherlands',
        },

    {
        'name': 'Feyenoord',
        'founded': datetime.datetime(1908, 7, 19),
        'city': 'Rotterdam, Netherlands',
        },

    {
        'name': 'Sparta Rotterdam',
        'founded': datetime.datetime(1888, 4, 1),
        'city': 'Rotterdam, Netherlands',
        },

    
    {
        'name': 'SV Wageningen',
        'city': 'Wageningen, Netherlands',
        },

    {
        'name': 'VV Steenwijk',
        'city': 'Steenwijk, Netherlands',
        },
    
    {
        'name': 'FC Groningen',
        'founded': datetime.datetime(1971, 6, 16),
        'city': 'Groningen, Netherlands',
        },

    {
        'name': 'Fortuna Sittard',
        'founded': datetime.datetime(1968, 7, 1),
        'city': 'Sittard, Netherlands',
        },
    
    {
        'name': 'FC Dordrecht',
        'founded': datetime.datetime(1883, 8, 16),
        'city': 'Dordrecht, Netherlands',
        },
    
    {
        'name': 'Telstar',
        'founded': datetime.datetime(1963, 7, 17),
        'city': 'Velsen, Netherlands',
        },
    
    {
        'name': 'FC Utrecht',
        'founded': datetime.datetime(1970, 7, 1),
        'city': 'Utrecht, Netherlands',
        },

    {
        'name': 'Willem II',
        'founded': datetime.datetime(1896, 8, 12),
        'city': 'Tilburg, Netherlands',
        },

    {
        'name': 'Roda JC',
        'founded': datetime.datetime(1962, 6, 27),
        'city': 'Kerkrade, Netherlands',
        },

    {
        'name': 'VVV-Venlo',
        'founded': datetime.datetime(1903, 2, 7),
        'city': 'Venlo, Netherlands',
        },

    {
        'name': 'FC Den Bosch',
        'founded': datetime.datetime(1965, 8, 18),
        'city': '\'s-Hertogenbosch, Netherlands',
        },

    {
        'name': 'ADO Den Haag',
        'founded': datetime.datetime(1905, 2, 1),
        'city': 'The Hague, Netherlands',
        },

    {
        'name': 'MVV Maastricht',
        'founded': datetime.datetime(1902, 4, 2),
        'city': 'Maastricht, Netherlands',
        },

    {
        'name': 'Almere City FC',
        'founded': datetime.datetime(2001, 9, 14),
        'city': 'Almere, Netherlands',
        },

    {
        'name': 'Helmond Sport',
        'founded': datetime.datetime(1967, 7, 27),
        'city': 'Helmond, Netherlands',
        },

    {
        'name': 'NAC Breda',
        'founded': datetime.datetime(1912, 9, 19),
        'city': 'Breda, Netherlands',
        },

    {
        'name': 'Go Ahead Eagles',
        'founded': datetime.datetime(1902, 12, 2),
        'city': 'Deventer, Netherlands',
        },

    {
        'name': 'RBC Roosendaal',
        'founded': datetime.datetime(1927, 7, 16),
        'city': 'Roosendaal, Netherlands',
        },

    {
        'name': 'FC Emmen',
        'founded': datetime.datetime(1925, 8, 21),
        'city': 'Emmen, Netherlands',
        },




    # Hungary

    {
        'name': 'MTK Budapest',
        'founded': datetime.datetime(1888, 11, 16),
        'city': 'Budapest, Hungary',
        },

    {
        'name': 'III. Kerületi TUE',
        'founded': datetime.datetime(1887, 1, 24),
        'city': 'Budapest, Hungary',
        },
    {
        'name': 'FC Sopron',
        'founded': 1921,
        'dissolved': 2008,
        'city': 'Sopron, Hungary',
        },

    {
        'name': 'Rákospalotai EAC',
        'founded': 1912,
        'city': 'Budapest, Hungary',
        },

    {
        'name': 'Zalaegerszegi TE',
        'founded': 1920,
        'city': 'Zalaegerszeg, Hungary',
        },


    {
        'name': 'Ferencvaros',
        'founded': datetime.datetime(1899, 5, 3),
        'city': 'Budapest, Hungary',
        },

    {
        'name': 'BVSC Budapest',
        'founded': 1911,
        'dissolved': 2001,
        'city': 'Budapest, Hungary',
        },

    {
        'name': 'Tatabánya',
        'founded': 1910,
        'city': 'Tatabánya, Hungary',
        },

    {
        'name': 'Békéscsaba',
        'founded': datetime.datetime(1912, 10, 22),
        'city': 'Békéscsaba, Hungary',
        },

    {
        'name': 'Újpest FC',
        'founded': datetime.datetime(1885, 6, 16),
        'city': 'Budapest, Hungary',
        },

    {
        'name': 'Videoton',
        'founded': 1941,
        'city': 'Székesfehérvár, Hungary',
        },

    {
        'name': 'Haladás',
        'founded': 1919,
        'city': 'Szombathely, Hungary',
        },

    {
        'name': 'Egri FC',
        'founded': 1907,
        'city': 'Eger, Hungary',
        },

    {
        'name': 'Diósgyőr',
        'founded': datetime.datetime(1910, 2, 6),
        'city': 'Miskolc, Hungary',
        },

    {
        'name': 'Kecskemét',
        'founded': datetime.datetime(1911, 6, 11),
        'city': 'Kecskemét, Hungary',
        },

    {
        'name': 'Debrecen',
        'founded': datetime.datetime(1902, 3, 12),
        'city': 'Debrecen, Hungary',
        },

    {
        'name': 'Pécsi MFC',
        'founded': 1950,
        'city': 'Pécs, Hungary',
        },

    {
        'name': 'Honvéd',
        'founded': datetime.datetime(1909, 8, 3),
        'city': 'Budapest, Hungary',
        },

    {
        'name': 'Kaposvár',
        'founded': datetime.datetime(1923, 8, 15),
        'city': 'Kaposvár, Hungary',
        },

    {
        'name': 'Paks',
        'founded': datetime.datetime(1952, 11, 28),
        'city': 'Paks, Hungary',
        },

    {
        'name': 'Pápa',
        'founded': 1995,
        'city': 'Pápa, Hungary',
        },

    {
        'name': 'BFC Siófok',
        'founded': 1921,
        'city': 'Siófok, Hungary',
        },

    {
        'name': 'Győr',
        'founded': 1904,
        'city': 'Győr, Hungary',
        },

    {
        'name': 'Vác',
        'founded': 1899,
        'city': 'Vác, Hungary',
        },

    {
        'name': 'Nyíregyháza Spartacus',
        'founded': 1959,
        'city': 'Nyíregyháza, Hungary',
        },

    {
        'name': 'Dunaújváros FC',
        'founded': 1952,
        'dissolved': 2009,
        'city': 'Dunaújváros, Hungary',
        },

    {
        'name': 'Vasas SC',
        'founded': datetime.datetime(1911, 3, 16),
        'city': 'Budapest, Hungary',
        },



    # Greece




    {
        'name': 'Panionios',
        'founded': 1890,
        'city': 'Smyrna, Greece',
        },


    {
        'name': 'AEK Athens F.C.',
        'founded': 1924,
        'city': 'Athens, Greece',
        },

    {
        'name': 'Olympiakos',
        'founded': datetime.datetime(1925, 3, 10),
        'city': 'Piraeus, Greece',
        },

    {
        'name': 'Ethnikos Piraeus',
        'founded': 1923,
        'city': 'Piraeus, Greece',
        },


    {
        'name': 'Kallithea',
        'founded': 1966,
        'city': 'Kallithea, Greece',
        },
    {
        'name': 'Egaleo',
        'founded': 1931,
        'city': 'Egaleo, Greece',
        },

    {
        'name': 'PAOK',
        'founded': 1926,
        'city': 'Thessaloniki, Greece',
        },

    {
        'name': 'Iraklis',
        'founded': datetime.datetime(1908, 11, 29),
        'city': 'Thessaloniki, Greece',
        },

    {
        'name': 'Levadiakos',
        'founded': 1961,
        'city': 'Livadeia, Greece',
        },
    {
        'name': 'Ionikos',
        'founded': 1965,
        'city': 'Nikaia, Greece',
        },

    {
        'name': 'Ergotelis',
        'founded': datetime.datetime(1929, 8, 7),
        'city': 'Heraklion, Greece',
        },

    {
        'name': 'Skoda Xanthi',
        'founded': 1967,
        'city': 'Xanthi, Greece',
        },

    {
        'name': 'Akratitos',
        'founded': 1963,
        'city': 'Ano Liosia, Greece',
        },

    {
        'name': 'Apollon Kalamarias',
        'founded': 1926,
        'city': 'Thessaloniki, Greece',
        },

    {
        'name': 'Aris',
        'founded': datetime.datetime(1914, 3, 25),
        'city': 'Thessaloniki, Greece',
        },

    {
        'name': 'OFI',
        'founded': 1925,
        'city': 'Heraklion, Greece',
        },

    {
        'name': 'AEL 1964',
        'founded': datetime.datetime(1964, 5, 17),
        'city': 'Larissa, Greece',
        },

    {
        'name': 'Asteras Tripoli',
        'founded': 1931,
        'city': 'Tripoli, Greece',
        },

    {
        'name': 'PAS Giannina',
        'founded': 1966,
        'city': 'Ioannina, Greece',
        },

    {
        'name': 'Kavala',
        'founded': 1965,
        'city': 'Kavala, Greece',
        },

    {
        'name': 'Panathinaikos',
        'founded': datetime.datetime(1908, 2, 3),
        'city': 'Athens, Greece',
        },

    {
        'name': 'Thrasyvoulos',
        'founded': 1938,
        'city': 'Fyli, Greece',
        },

    {
        'name': 'Panserraikos',
        'founded': 1964,
        'city': 'Serres, Greece',
        },


    # Romania

    {
        'name': 'Dynamo Bucharest',
        'founded': datetime.datetime(1948, 5, 14),
        'city': 'Bucharest, Romania',
        },

    {
        'name': 'Ceahlăul Piatra Neamț',
        'founded': datetime.datetime(1919, 10, 20),
        'city': 'Piatra Neamț, Romania',
        },

    {
        'name': 'FC Vaslui',
        'founded': datetime.datetime(2002, 7, 20),
        'city': 'Vaslui, Romania',
        },

    {
        'name': 'Argeș Pitești',
        'founded': 1953,
        'dissolved': 2013,
        'city': 'Pitesti, Romania',
        },

    {
        'name': 'FC Steaua București',
        'founded': datetime.datetime(1947, 6, 7),
        'city': 'Bucharest, Romania',
        },

    {
        'name': 'FC Braşov',
        'founded': 1936,
        'city': 'Braşov, Romania',
        },

    {
        'name': 'FC Caracal',
        'founded': 1949,
        'city': 'Caracal, Romania',
        },

    {
        'name': 'Unirea Alba Iulia',
        'founded': 1924,
        'dissolved': 2013,
        'city': 'Alba Iulia, Romania',
        },

    {
        'name': 'Viitorul Constanța',
        'founded': 2009,
        'city': 'Constanța, Romania',
        },

    {
        'name': 'Oțelul Galați',
        'founded': 1964,
        'city': 'Galați, Romania',
        },




    # Israel



    {
        'name': 'Maccabi Ironi Ashdod',
        'founded': 1961,
        'dissolved': 1999,
        'city': 'Ashdod, Israel',
        },

    {
        'name': 'Maccabi Tel Aviv',
        'founded': 1906,
        'city': 'Tel Aviv, Israel',
        },

    {
        'name': 'Hapoel Petah Tikva F.C.',
        'founded': 1934,
        'city': 'Petah Tikva, Israel',
        },


    {
        'name': 'Hapoel Tel Aviv F.C.',
        'founded': datetime.datetime(1927, 5, 31),
        'city': 'Tel Aviv, Israel',
        },


    # Croatia

    {
        'name': 'Hajduk Split',
        'founded': datetime.datetime(1911, 2, 13),
        'city': 'Split, Croatia',
        },

    {
        'name': 'RNK Split',
        'founded': datetime.datetime(1912, 4, 16),
        'city': 'Split, Croatia',
        },

    {
        'name': 'Dinamo Zagreb',
        'founded': datetime.datetime(1945, 5, 9),
        'city': 'Zagreb, Croatia',
        },

    {
        'name': 'NK Lokomotiva',
        'founded': datetime.datetime(1914, 5, 1),
        'city': 'Zagreb, Croatia',
        },

    {
        'name': 'HNK Cibalia',
        'founded': 1919, 
        'city': 'Vinkovci, Croatia',
        },

    # Northern Ireland


    {
        'name': 'Lisburn Distillery',
        'city': 'Lisburn, Northern Ireland',
        'founded': 1880,
        },

    {
        'name': 'Portadown F.C.',
        'city': 'Portadown, Northern Ireland',
        'founded': 1888,
        },




    {
        'name': 'Glenavon FC',
        'founded': 1889,
        'city': 'Lurgan, Northern Ireland',
        },
    {
        'name': 'Crusaders Belfast',
        'founded': 1898,
        'city': 'Belfast, Northern Ireland',
        },

    {
        'name': 'Glentoran',
        'founded': 1882,
        'city': 'Belfast, Northern Ireland',
        },

    {
        'name': 'Derry City',
        'founded': 1928,
        'city': 'Derry, Northern Ireland',
        },
    {
        'name': 'Coleraine F.C.',
        'founded': 1927,
        'city': 'Coleraine, Northern Ireland',
        },
    {
        'name': 'Ards F.C.',
        'founded': 1900,
        'city': 'Newtownards, Northern Ireland',
        },
    {
        'name': 'Linfield F.C.',
        'founded': 1886,
        'city': 'Belfast, Northern Ireland',
        },
    {
        'name': 'Larne F.C.',
        'founded': 1889,
        'city': 'Larne, Northern Ireland',
        },

    # Serbia
    {
        'name': 'Red Star Belgrade',
        'founded': datetime.datetime(1945, 3, 4),
        'city': 'Belgrade, Serbia',
        },

    {
        'name': 'Partizan Belgrade',
        'founded': datetime.datetime(1945, 10, 4),
        'city': 'Belgrade, Serbia',
        },





# Luxembourg

    {
        'name': 'Jeunesse d\'Esch',
        'city': 'Esch-sur-Alzette, Luxembourg',
        'founded': 1907,
        },


    {
        'name': 'Aris Bonnevoie',
        'city': 'Luxembourg City, Luxembourg',
        'founded': 1922,
        },

    {
        'name': 'Progrès Niedercorn',
        'city': 'Niederkorn, Luxembourg',
        'founded': 1919,
        },



    {
        'name': 'Red Boys Differdange',
        'city': 'Differdange, Luxembourg',
        'founded': 1907,
        },


    {
        'name': 'Spora Luxembourg',
        'city': 'Luxembourg City, Luxembourg',
        'founded': 1923,
        'dissolved': 2005,
        },

    {
        'name': 'Union Luxembourg',
        'city': 'Luxembourg City, Luxembourg',
        'founded': 1925,
        'dissolved': 2005,
        },

    {
        'name': 'Stade Dudelange',
        'city': 'Dudelange, Luxembourg',
        'founded': 1913,
        'dissolved': 1991,
        },



    {
        'name': 'FC Avenir Beggen',
        'founded': datetime.datetime(1915, 7, 1),
        'city': 'Luxembourg City, Luxembourg',
        },


    # Belgium


    {
        'name': 'RWD Molenbeek',
        'city': 'Brussels, Belgium',
        'founded': 1973,
        'dissolved': 2002,
        },

    {
        'name': 'Boussu Dour Borinage',
        'city': 'Boussu, Belgium',
        'founded': 1922,
        },

    {
        'name': 'RWDM Brussels',
        'city': 'Brussels, Belgium',
        'founded': datetime.datetime(1932, 11, 30),
        },

    {
        'name': 'Oudenarde',
        'city': 'Oudenarde, Belgium',
        'founded': 1911,
        },


    {
        'name': 'KRC Harelbeke',
        'city': 'Harelbeke, Belgium',
        'founded': 1930,
        'dissolved': 2002,
        },

    {
        'name': 'Eendracht Aalst',
        'city': 'Aalst, Belgium',
        'founded': datetime.datetime(1919, 6, 25),
        },

    {
        'name': 'Heusden-Zolder',
        'city': 'Heusden-Zolder, Belgium',
        'founded': 1936,
        'dissolved': 2006,
        },

    {
        'name': 'Geel', #K.F.C. Verbroedering Geel
        'city': 'Antwerp, Belgium',
        'founded': 1912,
        },


    {
        'name': 'Charleroi SC',
        'founded': 1904,
        'city': 'Charleroi, Belgium',
        },


    {
        'name': 'KFC Lommelse',
        'founded': 1932,
        'dissolved': 2003,
        'city': 'Lommel, Belgium',
        },

    {
        'name': 'Mons',
        'founded': 1909,
        'city': 'Mons, Belgium',
        },

    {
        'name': 'Roeselare',
        'founded': datetime.datetime(1921, 7, 20),
        'city': 'Roeselare, Belgium',
        },

    {
        'name': 'Oud-Heverlee Leuven',
        'founded': 2002,
        'city': 'Leuven, Belgium',
        },


    {
        'name': 'R.F.C. Seraing',
        'founded': 1900,
        'city': 'Seraing, Belgium',
        },

    {
        'name': 'Tubize',
        'founded': datetime.datetime(1953, 2, 2),
        'city':  'Tubize, Belgium',
        },

    {
        'name': 'Westerlo',
        'founded': datetime.datetime(1933, 9, 5),
        'city': 'Westerlo, Belgium',
        },

    {
        'name': 'Mouscron',
        'founded': datetime.datetime(1964, 7, 1),
        'dissolved': datetime.datetime(2009, 12, 28),
        'city': 'Mouscron, Belgium',
        },

    {
        'name': 'Mouscron-Péruwelz',
        'founded': datetime.datetime(1921, 7, 10),
        'city': 'Mouscron, Belgium',
        },

    {
        'name': 'Lokeren',
        'founded': datetime.datetime(1923, 1, 22),
        'city': 'Lokeren, Belgium',
        },

    {
        'name': 'Zulte Waregem',
        'founded': 1950,
        'city': 'Waregem, Belgium',
        },

    {
        'name': 'KSV Waregem',
        'founded': 1925,
        'dissolved': 2001,
        'city': 'Waregem, Belgium',
        },

    {
        'name': 'Oostende',
        'founded': 1981,
        'city': 'Ostend, Belgium',
        },

    {
        'name': 'KV Kortrijk',
        'founded': 1901,
        'city': 'Kortrijk, Belgium',
        },

    {
        'name': 'Sint-Truiden',
        'founded': datetime.datetime(1924, 2, 23),
        'city': 'Sint-Truiden, Belgium',
        },

    {
        'name': 'Gent',
        'founded': 1864,
        'city': 'Ghent, Belgium',
        },

    {
        'name': 'Waasland-Beveren',
        'founded': 1936,
        'city': 'Beveren, Belgium',
        },

    {
        'name': 'KSK Beveren',
        'founded': datetime.datetime(1934, 7, 1),
        'city': 'Beveren, Belgium',
        },

    {
        'name': 'Dinamo Tbilisi',
        'founded': 1925,
        'city': 'Tbilisi, Georgia',
        },

    {
        'name': 'Ħamrun Spartans',
        'founded': 1907,
        'city': 'Ħamrun, Malta',
        },

    {
        'name': 'Rabat Ajax',
        'founded': 1930,
        'city': 'Rabat, Malta',
        },

    {
        'name': 'Beerschot AC',
        'founded': 1999,
        'dissolved': 2013,
        'city': 'Antwerp, Belgium',
        },

    {
        'name': 'Club Brugge',
        'founded': datetime.datetime(1891, 11, 13),
        'city': 'Bruges, Belgium',
        },

    {
        'name': 'Cercle Brugge',
        'founded': 1899,
        'city': 'Bruges, Belgium',
        },

    # Belgium
    {
        'name': 'Royal Antwerp F.C.',
        'founded': 1880,
        'city': 'Antwerp, Belgium',
        },


    {
        'name': 'RSC Anderlecht',
        'founded': datetime.datetime(1908, 5, 27),
        'city': 'Anderlecht, Belgium',
        },


    {
        'name': 'Standard Liege',
        'founded': 1898,
        'city': 'Liege, Belgium',
        },

    {
        'name': 'RFC Liège',
        'founded': 1892,
        'city': 'Liege, Belgium',
        },

    {
        'name': 'Eupen',
        'founded': 1945,
        'city': 'Eupen, Belgium',
        },

    {
        'name': 'Dender EH',
        'founded': 1935,
        'city': 'Denderleeuw, Belgium',
        },

    {
        'name': 'Dessel',
        'founded': 1926,
        'city': 'Dessel, Belgium',
        },

    {
        'name': 'Lierse SK',
        'founded': datetime.datetime(1906, 3, 6),
        'city': 'Lier, Belgium',
        },

    {
        'name': 'Racing Genk',
        'founded': 1988,
        'city': 'Genk, Belgium',
        },

    {
        'name': 'Arles-Avignon',
        'founded': 1913,
        'city': 'Avignon, Belgium',
        },

    {
        'name': 'KV Mechelen',
        'founded': 1904,
        'city': 'Mechelen, Belgium',
        },

    {
        'name': 'La Louviere',
        'city': 'La Louviere, Belgium',
        },


    # Spain

    {
        'name': 'Fayal SC',
        'city': 'Faial, Spain',
        },

    {
        'name': 'UD Las Palmas',
        'founded': 1949,
        'city': 'Las Palmas de Gran Canaria, Spain',
        },

    {
        'name': 'Real Oviedo',
        'founded': datetime.datetime(1926, 3, 26),
        'city': 'Oviedo, Spain',
        },

    {
        'name': 'Rayo Vallecano',
        'founded': 1924,
        'city': 'Madrid, Spain',
        },

    {
        'name': 'SD Compostela',
        'founded': 1962,
        'city': 'Santiago de Compostela, Spain',
        },

    {
        'name': 'Celta Vigo',
        'founded': datetime.datetime(1923, 8, 23),
        'city': 'Vigo, Spain',
        },

    {
        'name': 'Mallorca',
        'founded': 1916,
        'city': 'Palma, Spain',
        },

    {
        'name': 'Real Betis',
        'founded': datetime.datetime(1907, 9, 12),
        'city': 'Seville, Spain',
        },

    {
        'name': 'Recreativo Huelva',
        'founded': 1889,
        'city': 'Huelva, Spain',
        },

    {
        'name': 'Athletic Bilbao',
        'founded': 1898,
        'city': 'Bilbao, Spain',
        },

    {
        'name': 'Getafe',
        'founded': 1983,
        'city': 'Getafe, Spain',
        },

    {
        'name': 'Numancia',
        'founded': 1945,
        'city': 'Soria, Spain',
        },

    {
        'name': 'Almería',
        'founded': 1989,
        'city': 'Almeria, Spain',
        },

    {
        'name': 'Cordoba CF',
        'founded': 1954,
        'city': 'Cordoba, Spain',
        },

    {
        'name': 'Levante',
        'founded': 1909,
        'city': 'Valencia, Spain',
        },

    {
        'name': 'Albacete',
        'founded': 1984,
        'city': 'Albacete, Spain',
        },

    {
        'name': 'Real Sociedad',
        'founded': datetime.datetime(1909, 9, 7),
        'city': 'San Sebastian, Spain',
        },

    {
        'name': 'Osasuna',
        'founded': datetime.datetime(1920, 10, 24),
        'city': 'Pamplona, Spain',
        },

    {
        'name': 'Coria CF',
        'founded': 1923,
        'city': 'Coria del Rio, Spain',
        },

    {
        'name': 'FC Barcelona',
        'founded': datetime.datetime(1899, 11, 29),
        'city': 'Barcelona, Spain',
        },

    {
        'name': 'RCD Espanyol',
        'founded': datetime.datetime(1900, 10, 28),
        'city': 'Barcelona, Spain',
        },

    {
        'name': 'CE Sabadell FC',
        'founded': 1903,
        'city': 'Sabadell, Spain',
        },

    {
        'name': 'Real Valladolid',
        'founded': 1928,
        'city': 'Valladolid, Spain',
        },


    {
        'name': 'Girona FC',
        'founded': 1930,
        'city': 'Girona, Spain',
        },

    {
        'name': 'Deportivo Alavés',
        'founded': 1921,
        'city': 'Vitoria-Gasteiz, Spain',
        },



    {
        'name': 'Sporting de Gijon',
        'founded': 1905,
        'city': 'Gijon, Spain',
        },

    {
        'name': 'Hercules CF',
        'founded': 1922,
        'city': 'Alicante, Spain',
        },

    {
        'name': 'Málaga',
        'founded': datetime.datetime(1904, 4, 3),
        'city': 'Malaga, Spain',
        },

    {
        'name': 'Villarreal',
        'founded': datetime.datetime(1923, 3, 10),
        'city': 'Villarreal, Spain',
        },

    {
        'name': 'Real Madrid',
        'founded': datetime.datetime(1902, 3, 6),
        'city': 'Madrid, Spain',
        },


    {
        'name': 'Deportivo La Coruña',
        'founded': 1906,
        'city': 'A Coruña, Spain',
        },

    {
        'name': 'Racing de Santander',
        'founded': 1913,
        'city': 'Santander, Spain',
        },

    
    {
        'name': 'Valencia CF',
        'founded': datetime.datetime(1919, 3, 18),
        'city': 'Valencia, Spain',
        },


    {
        'name': 'Real Zaragoza',
        'founded': datetime.datetime(1932, 3, 18),
        'city': 'Zaragoza, Spain',
        },

    {
        'name': 'Real Murcia',
        'founded': 1908,
        'city': 'Murcia, Spain',
        },
    {
        'name': 'Rabo de Peixe',
        'founded': 1985,
        'city': 'Rabo de Peixe, Spain',
        },

    {
        'name': 'Atlético Madrid',
        'founded': datetime.datetime(1903, 4, 26),
        'city': 'Madrid, Spain',
        },

    {
        'name': 'CF Extremadura',
        'founded': 1924,
        'dissolved': 2010,
        'city': 'Almendralejo, Spain',
        },



    {
        'name': 'Cádiz',
        'founded': 1910,
        'city': 'Cádiz, Spain',
        },


    {
        'name': 'Tenerife',
        'city': 'Santa Cruz de Tenerife, Spain',
        'founded': 1912,
        },

    {
        'name': 'Xerez',
        'city': 'Jerez de la Frontera, Spain',
        'founded': 1947,
        },


    {
        'name': 'Sevilla FC',
        'founded': datetime.datetime(1905, 10, 14),
        'city': 'Seville, Spain',
        },

    {
        'name': 'UD Salamanca',
        'founded': 1923,
        'dissolved': 2013,
        'city': 'Salamanca, Spain',
        },



    {
        'name': 'RSD Alcalá',
        'founded': 1929,
        'city': 'Alcala de Henares, Spain',
        },






    # Cyprus


    {
        'name': 'APOEL Nicosia',
        'city': 'Nicosia, Cyprus',
        'founded': datetime.datetime(1926, 11, 8),
        },

    {
        'name': 'Olympiakos Nicosia',
        'city': 'Nicosia, Cyprus',
        'founded': 1931,
        },

    {
        'name': 'Omonia Nicosia',
        'city': 'Nicosia, Cyprus',
        'founded': datetime.datetime(1948, 6, 4),
        },

    {
        'name': 'Anorthosis Famagusta',
        'city': 'Famagusta, Cyprus',
        'founded': 1911,
        },

    {
        'name': 'EPA Larnaca',
        'city': 'Larnaca, Cyprus',
        'founded': 1930,
        'dissolved': 1994,
        },

    {
        'name': 'Pezoporikos Larnaca',
        'city': 'Larnaca, Cyprus',
        'founded': 1927,
        'dissolved': 1994,
        },

    {
        'name': 'Apollon Limassol',
        'founded': datetime.datetime(1954, 4, 14),
        'city': 'Limassol, Cyprus',
        },

    {
        'name': 'AEL Limassol',
        'founded': datetime.datetime(1930, 10, 4),
        'city': 'Limassol, Greece',
        },


    {
        'name': 'Panthrakikos',
        'founded': 1963,
        'city': 'Komotini, Cyprus',
        },

    {
        'name': 'Veria',
        'founded': 1960,
        'city': 'Veroia, Cyprus',
        },

    {
        'name': 'Platanias',
        'founded': datetime.datetime(1931, 3, 15),
        'city': 'Platanias, Cyprus',
        },


    # Austria


    {
        'name': 'Wacker Innsbruck',
        'city': 'Innsbruck, Austria',
        'founded': 1915,
        'dissolved': 1999,
        },

    {
        'name': 'FC Linz',
        'city': 'Linz, Austria',
        'founded': 1946,
        'dissolved': 1997,
        },


    {
        'name': 'Austria Kärnten',
        'city': 'Klagenfurt, Austria',
        'founded': 2007,
        'dissolved': 2010,
        },

    {
        'name': 'SV Ried',
        'city': 'Ried im Innkreis, Austria',
        'founded': 1912,
        },

    {
        'name': 'Admira Wacker Mödling',
        'city': 'Mödling, Austria',
        'founded': 1905,
        },

    {
        'name': 'Wolfsberger AC',
        'city': 'Wolfsberg, Austria',
        'founded': 1931,
        },

    {
        'name': 'Vorwärts Steyr',
        'city': 'Steyr, Austria',
        'founded': 1919,
        },



    {
        'name': 'Austria Wien',
        'founded': datetime.datetime(1911, 3, 12),
        'city': 'Vienna, Austria',
        },

    {
        'name': 'Sturm Graz',
        'founded': 1909,
        'city': 'Graz, Austria',
        },

    {
        'name': 'Grazer AK',
        'founded': 1902,
        'dissolved': datetime.datetime(2012, 10, 29),
        'city': 'Graz, Austria',
        },


    {
        'name': 'Rapid Vienna',
        'founded': 1899,
        'city': 'Vienna, Austria',
        },

    {
        'name': 'Kapfenberg',
        'founded': datetime.datetime(1919, 9, 14),
        'city': 'Kapfenberg, Austria',
        },

    {
        'name': 'Austria Lustenau',
        'founded': 1914,
        'city': 'Lustenau, Austria',
        },

    {
        'name': 'FC Red Bull Salzburg',
        'founded': 1933,
        'city': 'Salzburg, Austria',
        },


    {
        'name': 'Austria Salzburg',
        'founded': datetime.datetime(2005, 10, 7),
        'city': 'Salzburg, Austria',
        },


    {
        'name': 'Wiener Neustadt',
        'founded': datetime.datetime(2008, 5, 19),
        'city': 'Wiener Neustadt, Austria',
        },

    {
        'name': 'Wiener Sport-Club',
        'founded': 1883,
        'city': 'Vienna, Austria',
        },

    {
        'name': 'LASK Linz',
        'founded': datetime.datetime(1908, 8, 7),
        'city': 'Linz, Austria',
        },

    {
        'name': 'FC Pasching',
        'founded': datetime.datetime(2007, 5, 16),
        'city': 'Pasching, Austria',
        },

    {
        'name': 'SV Mattersburg',
        'founded': 1922,
        'city': 'Mattersburg, Austria',
        },

    {
        'name': 'SV Schwechat',
        'founded': 1903,
        'city': 'Schwechat, Austria',
        },

    {
        'name': 'Rheindorf Altach',
        'founded': datetime.datetime(1929, 12, 26),
        'city': 'Altach, Austria',
        },

    
    # Switzerland

    {
        'name': 'FC Locarno',
        'founded': 1906,
        'city': 'Locarno, Switzerland',
        },

    {
        'name': 'FC La Chaux-de-Fonds',
        'founded': 1894,
        'city': 'La Chaux-de-Fonds, Switzerland',
        },

    {
        'name': 'FC Wil',
        'founded': 1900,
        'city': 'Wil, Switzerland',
        },

    {
        'name': 'Étoile Carouge',
        'founded': 1904,
        'city': 'Carouge, Switzerland',
        },

    {
        'name': 'Servette FC',
        'founded': datetime.datetime(1890, 3, 20),
        'city': 'Geneva, Switzerland',
        },

    {
        'name': 'Schaffhausen',
        'founded': 1896,
        'city': 'Schaffhausen, Switzerland',
        },

    {
        'name': 'FC Stade Lausanne',
        'city': 'Lausanne, Switzerland',
        },


    {
        'name': 'Lausanne-Sport',
        'city': 'Lausanne, Switzerland',
        'founded': 1896,
        },

    {
        'name': 'Grasshoppers',
        'founded': datetime.datetime(1886, 9, 1),
        'city': 'Zurich, Switzerland',
        },

    {
        'name': 'BSC Young Boys',
        'founded': datetime.datetime(1898, 3, 14),
        'city': 'Bern, Switzerland',
        },

    {
        'name': 'Yverdon-Sport FC',
        'founded': 1948,
        'city': 'Yverdon-les-Bains, Switzerland',
        },

    {
        'name': 'FC Birsfelden',
        'founded': datetime.datetime(1920, 7, 1),
        'city': 'Basel, Switzerland',
        },

    {
        'name': 'FC Sion',
        'founded': 1909,
        'city': 'Sion, Switzerland',
        },




    {
        'name': 'St. Gallen',
        'founded': datetime.datetime(1879, 4, 19),
        'city': 'St. Gallen, Switzerland',
        },

    {
        'name': 'Vaduz',
        'founded': 1932,
        'city': 'Vaduz, Liechtenstein',
        },

    {
        'name': 'Bellinzona',
        'founded': 1904,
        'dissolved': 2013,
        'city': 'Bellinzona, Switzerland',
        },


    {
        'name': 'Basel',
        'founded': datetime.datetime(1893, 11, 15),
        'city': 'Basel, Switzerland',
        },


    {
        'name': 'Neuchâtel Xamax',
        'founded': 1970,
        'city': 'Neuchâtel, Switzerland',
        },
    {
        'name': 'FC Zurich',
        'founded': 1896,
        'city': 'Zurich, Switzerland',
        },


    {
        'name': 'FC Swarovski Tirol',
        'founded': 1986,
        'dissolved': 1992,
        'city': 'Innsbruck, Switzerland',
        },

    {
        'name': 'FC Tirol Innsbruck',
        'founded': 1993,
        'dissolved': 2002,
        'city': 'Innsbruck, Switzerland',
        },


    {
        'name': 'Aarau',
        'founded': 1902,
        'city': 'Aarau, Switzerland',
        },

    {
        'name': 'FC Solothurn',
        'founded': datetime.datetime(1901, 7, 1),
        'city': 'Solothurn, Switzerland',
        },

    {
        'name': 'Thun',
        'founded': 1898,
        'city': 'Thun, Switzerland',
        },

    {
        'name': 'FC Luzern',
        'founded': datetime.datetime(1901, 8, 12),
        'city': 'Lucerne, Switzerland',
        },

    {
        'name': 'FC Lugano',
        'founded': 1908,
        'city': 'Lugano, Switzerland',
        },


    # Czech Republic


    {
        'name': 'Dukla Prague',
        'founded': 1948,
        #'dissolved': 1996,
        'city': 'Prague, Czech Republic',
        },

    {
        'name': 'Slavia Prague',
        'founded': datetime.datetime(1892, 11, 2),
        'city': 'Prague, Czech Republic',
        },

    {
        'name': 'SK Benešov',
        'founded': 1913,
        'city': 'Benešov, Czech Republic',
        },



    {
        'name': 'Sparta Prague',
        'founded': datetime.datetime(1893, 11, 16),
        'city': 'Prague, Czech Republic',
        },

    {
        'name': 'FC Vitkovice',
        'founded': 1919,
        'city': 'Vitkovice, Czech Republic',
        },

    {
        'name': 'Baník Ostrava',
        'founded': 1922,
        'city': 'Ostrava, Czech Republic',
        },

    {
        'name': 'Slovácko',
        'founded': 1927,
        'city': 'Uherské Hradiště, Czech Republic',
        },

    {
        'name': 'Zbrojovka Brno',
        'founded': 1913,
        'city': 'Brno, Czech Republic',
        },
    {
        'name': 'Dynamo České Budějovice',
        'founded': 1905,
        'city': 'České Budějovice, Czech Republic',
        },
    {
        'name': 'Sigma Olomouc',
        'founded': 1919,
        'city': 'Olomouc, Czech Republic',
        },

    {
        'name': 'FK Příbram',
        'founded': 1928,
        'city': 'Příbram, Czech Republic',
        },

    {
        'name': 'Slovan Liberec',
        'founded': 1958,
        'city': 'Liberec, Czech Republic',
        },

    {
        'name': 'FK Baumit Jablonec',
        'founded': 1945,
        'city': 'Jablonec nad Nisou, Czech Republic',
        },

    {
        'name': 'Viktoria Plzeň',
        'founded': 1911,
        'city': 'Plzen, Czech Republic',
        },

    {
        'name': 'Hradec Králové',
        'founded': 1905,
        'city': 'Hradec Králové, Czech Republic',
        },

    {
        'name': 'Teplice',
        'founded': 1945,
        'city': 'Teplice, Czech Republic',
        },

    {
        'name': 'Mladá Boleslav',
        'founded': 1902,
        'city': 'Mladá Boleslav, Czech Republic',
        },

    {
        'name': 'Vysočina Jihlava',
        'founded': 1948,
        'city': 'Jihlava, Czech Republic',
        },

    {
        'name': 'FC Fastav Zlín',
        'founded': 1919,
        'city': 'Zlin, Czech Republic',
        },

    {
        'name': 'Viktoria Žižkov',
        'founded': 1903,
        'city': 'Prague, Czech Republic',
        },

    {
        'name': 'SK Kladno',
        'founded': 1903,
        'city': 'Kladno, Czech Republic',
        },

    {
        'name': 'FK Ústí nad Labem',
        'founded': 1945,
        'city': 'Ústí nad Labem, Czech Republic',
        },

    {
        'name': 'Bohemians 1905',
        'founded': 1905,
        'city': 'Prague, Czech Republic',
        },

    {
        'name': 'Baník Most',
        'founded': 1909,
        'city': 'Most, Czech Republic',
        },

    {
        'name': 'Bohemians Prague (Střížkov)',
        'founded': 1996,
        'city': 'Prague, Czech Republic',
        },

    {
        'name': 'FK Drnovice',
        'founded': 1932,
        'city': 'Drnovice, Czech Republic',
        },

    {
        'name': 'SFC Opava',
        'founded': 1907,
        'city': 'Opava, Czech Republic',
        },

    {
        'name': 'MFK Karviná',
        #'founded': 2003,
        'city': 'Karviná, Czech Republic',
        },

    {
        'name': 'Blšany',
        'founded': 1946,
        'city': 'Blšany, Czech Republic',
        },

    # Turkey

    {
        'name': 'Besiktas',
        'founded': datetime.datetime(1903, 3, 19),
        'city': 'Istanbul, Turkey',
        },

    {
        'name': 'Orduspor',
        'founded': datetime.datetime(1967, 9, 19),
        'city': 'Ordu, Turkey',
        },

    {
        'name': 'Sarıyer G.K.',
        'founded': 1940,
        'city': 'Istanbul, Turkey',
        },

    {
        'name': 'Kardemir Karabükspor',
        'founded': 1969,
        'city': 'Karabük, Turkey',
        },

    {
        'name': 'Erzurumspor',
        'founded': 1968,
        'city': 'Erzurum, Turkey',
        },

    {
        'name': 'Elazığspor',
        'founded': 1967,
        'city': 'Elazığ, Turkey',
        },

    {
        'name': 'Istanbulspor',
        'founded': 1926,
        'city': 'Istanbul, Turkey',
        },

    {
        'name': 'Adanaspor',
        'founded': 1954,
        'city': 'Adana, Turkey',
        },

    {
        'name': 'Adana Demirspor',
        'founded': 1940,
        'city': 'Adana, Turkey',
        },

    {
        'name': 'Hacettepe',
        'founded': 1949,
        'city': 'Ankara, Turkey',
        },

    {
        'name': 'Goztepe',
        'founded': datetime.datetime(1925, 6, 14),
        'city': 'Izmir, Turkey',
        },


    {
        'name': 'Eskişehirspor',
        'founded': datetime.datetime(1965, 6, 19),
        'city': 'Eskisehir, Turkey',
        },

    {
        'name': 'Kasımpaşa',
        'founded': datetime.datetime(1921, 1, 15),
        'city': 'Beyoglu, Turkey',
        },

    {
        'name': 'İstanbul BB',
        'founded': datetime.datetime(1990, 6, 15),
        'city': 'Istanbul, Turkey',
        },

    {
        'name': 'Antalyaspor',
        'founded': datetime.datetime(1966, 7, 2),
        'city': 'Antalya, Turkey',
        },


    {
        'name': 'Galatasaray',
        'founded': datetime.datetime(1905, 10, 1),
        'city': 'Istanbul, Turkey',
        },

    {
        'name': 'Fenerbahce',
        'founded': datetime.datetime(1907, 5, 3),
        'city': 'Istanbul, Turkey',
        },

    {
        'name': 'Gençlerbirliği',
        'founded': datetime.datetime(1923, 3, 14),
        'city': 'Ankara, Turkey',
        },

    {
        'name': 'Trabzonspor',
        'founded': datetime.datetime(1967, 8, 2),
        'city': 'Trabzon, Turkey',
        },

    {
        'name': 'Ankaragücü',
        'founded': 1910,
        'city': 'Ankara, Turkey',
        },

    {
        'name': 'Dardanel S.A.Ş.',
        'founded': 1966,
        'city': 'Çanakkale, Turkey',
        },

    {
        'name': 'Ankaraspor',
        'founded': datetime.datetime(1978, 3, 21),
        'city': 'Ankara, Turkey',
        },

    {
        'name': 'Altay',
        'founded': datetime.datetime(1914, 1, 16),
        'city': 'Izmir, Turkey',
        },

    {
        'name': 'Diyarbakırspor',
        'founded': 1968,
        'city': 'Diyarbakir, Turkey',
        },

    {
        'name': 'Kocaelispor',
        'founded': 1966,
        'city': 'Izmit, Turkey',
        },

    {
        'name': 'Malatyaspor',
        'founded': 1966,
        'city': 'Malatya, Turkey',
        },

    {
        'name': 'Konyaspor',
        'founded': 1922,
        'city': 'Konya, Turkey',
        },

    {
        'name': 'Siirt S.K.',
        'founded': 1969,
        'city': 'Siirt, Turkey',
        },

    {
        'name': 'Samsunspor',
        'founded': datetime.datetime(1965, 6, 30),
        'city': 'Samsun, Turkey',
        },

    {
        'name': 'Kayseri Erciyesspor',
        'founded': datetime.datetime(1966, 6, 1),
        'city': 'Kayseri, Turkey',
        },

    {
        'name': 'Sakaryaspor',
        'founded': datetime.datetime(1965, 6, 17),
        'city': 'Sakarya, Turkey',
        },

    {
        'name': 'Kayserispor',
        'founded': 1975,
        'city': 'Kayseri, Turkey',
        },

    {
        'name': 'Denizlispor',
        'founded': datetime.datetime(1966, 5, 26),
        'city': 'Denizli, Turkey',
        },

    {
        'name': 'Denizli BSK',
        'founded': 1981,
        'city': 'Denizli, Turkey',
        },

    {
        'name': 'Bursaspor',
        'founded': datetime.datetime(1963, 6, 1),
        'city': 'Bursa, Turkey',
        },

    {
        'name': 'Manisaspor',
        'founded': datetime.datetime(1965, 6, 15),
        'city': 'Manisa, Turkey',
        },

    {
        'name': 'Rizespor',
        'founded': datetime.datetime(1953, 5, 19),
        'city': 'Rize, Turkey',
        },

    {
        'name': 'Gaziantepspor',
        'founded': datetime.datetime(1969, 2, 25),
        'city': 'Gaziantep, Turkey',
        },

    {
        'name': 'Akçaabat Sebatspor',
        'founded': 1923,
        'city': 'Trabzon, Turkey',
        },

    {
        'name': 'Akhisar Belediyespor',
        'founded': datetime.datetime(1970, 4, 8),
        'city': 'Akhisar, Turkey',
        },

    {
        'name': 'Bucaspor',
        'founded': datetime.datetime(1928, 3, 11),
        'city': 'Izmir, Turkey',
        },

    {
        'name': 'Karşıyaka',
        'founded': datetime.datetime(1912, 11, 1),
        'city': 'Karşıyaka, Turkey',
        },

    {
        'name': 'Mersin İdman Yurdu',
        'founded': datetime.datetime(1925, 8, 16),
        'city': 'Mersin, Turkey',
        },


    {
        'name': 'Şekerspor',
        'founded': 1947,
        'city': 'Ankara, Turkey',
        },

    {
        'name': 'Sivasspor',
        'founded': 1967,
        'city': 'Sivas, Turkey',
        },

    {
        'name': 'Vanspor',
        'founded': 1982,
        'city': 'Van, Turkey',
        },

    {
        'name': 'Zeytinburnuspor',
        'founded': 1953,
        'city': 'Istanbul, Turkey',
        },


    {
        'name': 'Yimpaş Yozgatspor',
        'founded': 1959,
        'city': 'Yozgat, Turkey',
        },

    # Italy

    {
        'name': 'Palermo',
        'founded': 1900,
        'city': 'Palermo, Italy',
        },

    {
        'name': 'US Sassuolo',
        'founded': 1922,
        'city': 'Palermo, Italy',
        },

    {
        'name': 'Varese',
        'founded': datetime.datetime(1910, 3, 22),
        'city': 'Varese, Italy',
        },

    {
        'name': 'Novara',
        'founded': 1908,
        'city': 'Novara, Italy',
        },


    {
        'name': 'Udinese',
        'founded': 1896,
        'city': 'Udine, Italy',
        },

    {
        'name': 'Salernitana',
        'founded': 1919,
        'city': 'Salerno, Italy',
        },

    {
        'name': 'Livorno',
        'founded': 1915,
        'city': 'Livorno, Italy',
        },

    {
        'name': 'Triestina',
        'founded': 1918,
        'city': 'Trieste, Italy',
        },

    {
        'name': 'Parma F.C.',
        'founded': datetime.datetime(1913, 12, 16),
        'city': 'Parma, Italy',
        },


    {
        'name': 'Arezzo',
        'founded': 1923, 
        'city': 'Arezzo, Italy',
        },

    {
        'name': 'A.S. Bari',
        'founded': 1908,
        'city': 'Bari, Italy',
        },

    {
        'name': 'Bologna F.C. 1909',
        'founded': 1909,
        'city': 'Bologna, Italy',
        },

    {
        'name': 'AFC Fiorentina',
        'founded': datetime.datetime(1926, 8, 26),
        'city': 'Florence, Italy',
        },

    {
        'name': 'Genoa CFC',
        'founded': datetime.datetime(1893, 9, 7),
        'city': 'Genoa, Italy',
        },

    {
        'name': 'Inter Milan',
        'founded': datetime.datetime(1908, 3, 9),
        'city': 'Milan, Italy',
        },

    {
        'name': 'Mantova F.C.',
        'founded': 1911, 
        'city': 'Mantua, Italy',
        },

    {
        'name': 'AC Milan',
        'founded': datetime.datetime(1899, 12, 16),
        'city': 'Milan, Italy',
        },

    {
        'name': 'Modena F.C.',
        'founded': 1912, 
        'city': 'Modena, Italy',
        },

    {
        'name': 'S.S.C. Napoli',
        'founded': 1926, 
        'city': 'Naples, Italy',
        },

    {
        'name': 'A.S. Roma',
        'founded': datetime.datetime(1927, 7, 22),
        'city': 'Rome, Italy',
        },

    {
        'name': 'U.C. Sampdoria',
        'founded': datetime.datetime(1946, 8, 1),
        'city': 'Genoa, Italy',
        },

    {
        'name': 'Venezia',
        'founded': 1907,
        'city': 'Venice, Italy',
        },

    {
        'name': 'Vicenza Calcio',
        'founded': datetime.datetime(1902, 3, 9),
        'city': 'Vicenza, Italy',
        },

    {
        'name': 'Empoli',
        'founded': 1920,
        'city': 'Empoli, Italy',
        },

    {
        'name': 'Reggina Calcio',
        'founded': 1914,
        'city': 'Reggio Calabria, Italy',
        },
    {
        'name': 'Piacenza',
        'founded': 1919,
        'city': 'Piacenza, Italy',
        },
    {
        'name': 'Perugia',
        'founded': 1905,
        'city': 'Perugia, Italy',
        },

    {
        'name': 'US Cantazaro 1929',
        'founded': 1929,
        'city': 'Catanzaro, Italy',
        },
    {
        'name': 'Cagliari',
        'founded': datetime.datetime(1920, 8, 20),
        'city': 'Cagliario, Italy',
        },
    {
        'name': 'Lecce',
        'founded': datetime.datetime(1908, 3, 15),
        'city': 'Lecce, Italy',
        },

    {
        'name': 'Lazio',
        'founded': datetime.datetime(1900, 1, 9),
        'city': 'Rome, Italy',
        },

    {
        'name': 'Brescia Calcio', 
        'founded': 1911,
        'city': 'Brescia, Italy',
        },

    {
        'name': 'Juventus',
        'founded': datetime.datetime(1897, 11, 1),
        'city': 'Turin, Italy',
        },

    {
        'name': 'AC Reggiana 1919',
        'founded': 1919,
        'city': 'Reggiana, Italy',
        },

    {
        'name': 'Verona',
        'founded': 1903,
        'city': 'Verona, Italy',
        },

    {
        'name': 'Atalanta',
        'founded': 1907,
        'city': 'Bergamo, Italy',
        },

    {
        'name': 'Torino FC',
        'founded': 1906,
        'city': 'Turin, Italy',
        },

    {
        'name': 'Chievo',
        'founded': 1929,
        'city': 'Chievo, Italy',
        },

    {
        'name': 'Como',
        'founded': 1907,
        'city': 'Como, Italy',
        },

    {
        'name': 'Siena',
        'founded': 1904,
        'city': 'Siena, Italy',
        },

    {
        'name': 'Ancona',
        'founded': 1905,
        'city': 'Ancona, Italy',
        },

    {
        'name': 'Messina',
        'founded': 1900,
        'city': 'Messina, Italy',
        },

    {
        'name': 'Ascoli',
        'founded': 1898,
        'city': 'Ascoli Piceno, Italy',
        },

    {
        'name': 'Treviso',
        'founded': 1909,
        'city': 'Treviso, Italy',
        },

    {
        'name': 'Catania',
        'founded': 1908,
        'city': 'Catania, Italy',
        },


    {
        'name': 'Cesena',
        'founded': 1940,
        'city': 'Cesena, Italy',
        },


    {
        'name': 'Avellino',
        'founded': 1912,
        'city': 'Aveillino, Italy',
        },


    # Ukraine


    {
        'name': 'Vorskla Poltava',
        'founded': 1955,
        'city': 'Poltava, Ukraine',
        },


    {
        'name': 'Stal Alchevsk',
        'founded': 1983,
        'city': 'Alchevsk, Ukraine',
        },

    {
        'name': 'Metalurh Zaporizhya',
        'founded': 1935,
        'city': 'Zaporizhya, Ukraine',
        },


    {
        'name': 'Tavriya Simferopol',
        'founded': 1958,
        'city': 'Simferopol, Ukraine',
        },

    {
        'name': 'Volyn Lutsk',
        'founded': 1960,
        'city': 'Lutsk, Ukraine',
        },

    {
        'name': 'Metalist Kharkiv',
        'founded': 1925,
        'city': 'Kharkiv, Ukraine',
        },

    {
        'name': 'Arsenal Kyiv',
        'founded': 2001,
        'dissolved': datetime.datetime(2013, 10, 29),
        'city': 'Kiev, Ukraine',
        },


    {
        'name': 'Dynamo Kiev',
        'founded': datetime.datetime(1927, 5, 13),
        'city': 'Kiev, Ukraine',
        },

    {
        'name': 'Obolon Kyiv',
        'founded': datetime.datetime(1992, 6, 13),
        'city': 'Kiev, Ukraine',
        },

    {
        'name': 'Chornomorets Odesa',
        'founded': datetime.datetime(1936, 3, 26),
        'city': 'Odessa, Ukraine',
        },

    {
        'name': 'FC Sevastopol',
        'founded': 2002,
        'city': 'Sevastopol, Ukraine',
        },

    {
        'name': 'Illichivets Mariupol',
        'founded': 1960,
        'city': 'Mariupol, Ukraine',
        },

    {
        'name': 'Naftovyk-Ukrnafta Okhtyrka',
        'founded': 1980,
        'city': 'Okhtyrka, Ukraine',
        },

    {
        'name': 'PFC Oleksandriya',
        'founded': 1948,
        'city': 'Oleksandriya, Ukraine',
        },

    {
        'name': 'Zorya Luhansk',
        'founded': 1923,
        'city': 'Luhansk, Ukraine',
        },

    {
        'name': 'Dnipro Dnipropetrovsk',
        'founded': 1918,
        'city': 'Dnipropetrovsk, Ukraine',
        },

    {
        'name': 'Shakhtar Donetsk',
        'founded': datetime.datetime(1936, 5, 24),
        'city': 'Donetsk, Ukraine',
        },
    {
        'name': 'Metalurh Donetsk',
        'founded': datetime.datetime(1996, 6, 17),
        'city': 'Donetsk, Ukraine',
        },


    {
        'name': 'FC Kharkiv',
        'founded': 2005,
        'city': 'Kharkiv, Ukraine',
        },

    {
        'name': 'FC Lviv',
        'founded': 2006,
        'dissolved': 2012,
        'city': 'Lviv, Ukraine',
        },

    {
        'name': 'Karpaty Lviv',
        'founded': datetime.datetime(1963, 1, 18),
        'city': 'Lviv, Ukraine',
        },

    {
        'name': 'Hoverla Uzhhorod',
        'founded': datetime.datetime(1925, 8, 15),
        'city': 'Uzhorod, Ukraine',
        },

    {
        'name': 'Kryvbas Kryvyi Rih',
        'founded': 1959,
        'dissolved': datetime.datetime(2013, 6, 12),
        'city': 'Kryvyi Rih, Ukraine',
        },

    # Belarus

    {
        'name': 'Dinamo Minsk',
        'founded': datetime.datetime(1927, 6, 18),
        'city': 'Minsk, Belarus',
        },






    # Portugal


    {
        'name': 'CU Micaelense',
        'founded': 1911,
        'city': 'Ponta Delgada, Portugal',
        },


    {
        'name': 'Leça FC',
        'founded': 1912,
        'city': 'Leça da Palmeira, Portugal',
        },

    {
        'name': 'CD Nacional',
        'founded': 1910,
        'city': 'Funchal, Portugal',
        },

    {
        'name': 'S.C. Caldas',
        'founded': 1916,
        'city': 'Caldas da Rainha, Portugal',
        },

    {
        'name': 'FC Porto',
        'founded': datetime.datetime(1893, 9, 28),
        'city': 'Porto, Portugal',
        },

    {
        'name': 'Boavista',
        'founded': 1903,
        'city': 'Porto, Portugal',
        },


    {
        'name': 'Braga',
        'founded': datetime.datetime(1921, 1, 19),
        'city': 'Braga, Portugal',
        },

    {
        'name': 'Trofense',
        'founded': datetime.datetime(1930, 9, 28),
        'city': 'Trofa, Portugal',
        },

    {
        'name': 'Leixões',
        'founded': 1907,
        'city': 'Matosinhos, Portugal',
        },

    {
        'name': 'Portimonense SC',
        'founded': datetime.datetime(1914, 8, 14),
        'city': 'Portimao, Portugal',
        },


    {
        'name': 'Académica de Coimbra',
        'founded': 1876,
        'city': 'Coimbra, Portugal',
        },

    {
        'name': 'Aves',
        'founded': datetime.datetime(1930, 11, 12),
        'city': 'Villa das Aves, Portugal',
        },

    {
        'name': 'Olhanense',
        'founded': 1912,
        'city': 'Algarve, Portugal',
        },

    {
        'name': 'S.L. Benfica',
        'founded': datetime.datetime(1904, 2, 28),
        'city': 'Lisbon, Portugal',
        },

    {
        'name': 'A.C. Marinhense',
        'founded': 1923,
        'city': 'Marinha Grande, Portugal',
        },

    {
        'name': 'C.S. Maritimo',
        'founded': datetime.datetime(2010, 9, 20),
        'city': 'Funchal, Portugal',
        },

    {
        'name': 'C.D. Santa Clara',
        'founded': datetime.datetime(1927, 5, 12),
        'city': 'Ponta Delgada, Portugal',
        },

    {
        'name': 'Sporting Lisbon',
        'founded': datetime.datetime(1906, 7, 1),
        'city': 'Lisbon, Portugal',
        },

    {
        'name': 'Varzim S.C.',
        'founded': datetime.datetime(1915, 12, 25),
        'city': 'Póvoa de Varzim, Portugal',
        },

    {
        'name': 'União de Santarém',
        'city': 'Santarém, Portugal',
        },

    {
        'name': 'C.F. Os Belenenses',
        'founded': datetime.datetime(1919, 9, 23),
        'city': 'Lisbon, Portugal',
        },

    {
        'name': 'Beira-Mar',
        'founded': 1922,
        'city': 'Aveiro, Portugal',
        },

    {
        'name': 'Clube Oriental de Lisboa',
        'founded': 1946,
        'city': 'Lisbon, Portugal',
        },

    {
        'name': 'UD Leiria',
        'founded': 1966,
        'city': 'Leiria, Portugal',
        },

    {
        'name': 'Feirense',
        'founded': datetime.datetime(1918, 3, 19),
        'city': 'Santa Maria da Feira, Portugal',
        },

    {
        'name': 'Salgueiros',
        'founded': datetime.datetime(1911, 12, 8),
        'city': 'Paranhos, Portugal',
        },

    {
        'name': 'Paços de Ferreira',
        'founded': 1950,
        'city': 'Paços de Ferreira, Portugal',
        },

    {
        'name': 'Vitória de Setúbal',
        'founded': datetime.datetime(1910, 11, 20),
        'city': 'Setubal, Portugal',
        },

    {
        'name': 'Gil Vicente',
        'founded': 1924,
        'city': 'Barcelos, Portugal',
        },

    {
        'name': 'Farense',
        'founded': datetime.datetime(1910, 4, 1),
        'city': 'Faro, Portugal',
        },

    {
        'name': 'Alverca',
        'founded': datetime.datetime(1939, 9, 1),
        'city': 'Alverca do Ribatejo, Portugal',
        },

    {
        'name': 'Vitória de Guimarães',
        'founded': 1922,
        'city': 'Guimaraes, Portugal',
        },

    {
        'name': 'Moreirense',
        'founded': 1938, 
        'city': 'Moreira de Cónegos, Portugal',
        },
    {
        'name': 'Rio Ave',
        'founded': 1939, 
        'city': 'Vila do Conde, Portugal',
        },

    {
        'name': 'Estoril',
        'founded': datetime.datetime(1939, 5, 17),
        'city': 'Estoril, Portugal',
        },

    {
        'name': 'Espinho',
        'founded': 1914,
        'city': 'Espinho, Portugal',
        },

    {
        'name': 'Estrela da Amadora',
        'founded': 1932, 
        'city': 'Jose Gomes, Portugal',
        },

    {
        'name': 'Penafiel',
        'founded': datetime.datetime(1951, 2, 8), 
        'city': 'Penafiel, Portugal',
        },

    {
        'name': 'FC Felgueiras',
        'founded': datetime.datetime(1936, 8, 16),
        'city': 'Felgueiras, Portugal',
        },

    {
        'name': 'FC Tirsense',
        'founded': 1938,
        'city': 'Santo Tirso, Portugal',
        },



    # France


    {
        'name': 'FC Girondins de Bordeaux',
        'founded': 1881, 
        'city': 'Bordeaux, France',
        },

    {
        'name': 'FC Vendenheim',
        'founded': 1974, 
        'city': 'Vendenheim, France',
        },

    {
        'name': 'ASJ Soyaux',
        'founded': 1968, 
        'city': 'Soyaux, France',
        },

    {
        'name': 'Yzeure Allier Auvergne',
        'founded': 1999, 
        'city': 'Yzeure, France',
        },

    {
        'name': 'Martigues',
        'founded': 1921, 
        'city': 'Martigues, France',
        },

    {
        'name': 'Gueugnon',
        'founded': 1940, 
        'city': 'Gueugnon, France',
        },

    {
        'name': 'Dijon FCO',
        'founded': 1998, 
        'city': 'Dijon, France',
        },

    {
        'name': 'Pescara',
        'founded': 1936, 
        'city': 'Pescara, France',
        },

    {
        'name': 'Berrichonne',
        'founded': 1883,
        'city': 'Châteauroux, France',
        },

    {
        'name': 'Cannes',
        'founded': 1902,
        'city': 'Cannes, France',
        },

    {
        'name': 'Marseille',
        'founded': 1899, 
        'city': 'Marseille, France',
        },

    {
        'name': 'AS Nancy',
        'founded': 1967,
        'city': 'Nancy, France',
        },

    {
        'name': 'FC Lorient',
        'founded': 1926,
        'city': 'Lorient, France',
        },


    {
        'name': 'FC Istres',
        'founded': 1920,
        'city': 'Fos-sur-Mer, France',
        },

    {
        'name': 'Guingamp',
        'founded': 1912,
        'city': 'Guingamp, France',
        },

    {
        'name': 'AS Monaco FC',
        'founded': datetime.datetime(1924, 8, 23),
        'city': 'Fontvielle, Monaco',
        },


    {
        'name': 'OGC Nice',
        'founded': 1904,
        'city': 'Nice, France',
        },


    {
        'name': 'Paris Saint-Germain',
        'founded': datetime.datetime(1970, 8, 12),
        'city': 'Paris, France',
        },


    
    {
        'name': 'Olympique Lyonnais',
        'founded': 1899,
        'city': 'Lyon, France',
        },

    
    {
        'name': 'Rennes',
        'founded': 1901,
        'city': 'Rennes, France',
        },

    {
        'name': 'Nantes',
        'founded': 1943,
        'city': 'Nantes, France',
        },

    {
        'name': 'Lens',
        'founded': 1906,
        'city': 'Lens, France',
        },
    
    {
        'name': 'Montpellier Herault SC',
        'founded': 1974,
        'city': 'Montpellier, France',
        },

    {
        'name': 'Metz',
        'founded': 1932,
        'city': 'Metz, France',
        },

    {
        'name': 'RC Strasbourg',
        'founded': 1906,
        'city': 'Strasbourg, France',
        },

    {
        'name': 'Bastia',
        'founded': 1905,
        'city': 'Bastia, France',
        },

    {
        'name': 'Lille OSC',
        'founded': datetime.datetime(1944, 9, 23),
        'city': 'Lille, France',
        },

    {
        'name': 'FC Sochaux',
        'founded': 1928,
        'city': 'Montbeliard, France',
        },




    {
        'name': 'Valenciennes FC',
        'founded': 1913,
        'city': 'Valenciennes, France',
        },

    {
        'name': 'Reims',
        'founded': 1911,
        'city': 'Rheims, France',
        },

    {
        'name': 'Brest',
        'founded': 1903,
        'city': 'Brest, France',
        },

    {
        'name': 'Caen',
        'founded': datetime.datetime(1913, 11, 17),
        'city': 'Caen, France',
        },

    {
        'name': 'Evian TG',
        'founded': 2003,
        'city': 'Gaillard, France',
        },


    {
        'name': 'Boulogne',
        'founded': 1898,
        'city': 'Boulogne-sur-Mer, France',
        },


    {
        'name': 'Grenoble',
        'founded': 1892,
        'city': 'Grenoble, France',
        },

    #{
    #    'name': 'Lyon',
    #    'founded': 1950,
    #    'city': 'Lyon, France',
    #    },

    {
        'name': 'Auxerre',
        'founded': 1905,
        'city': 'Auxerre, France',
        },

    {
        'name': 'Toulouse',
        'founded': 1937,
        'city': 'Toulouse, France',
        },


    {
        'name': 'Ajaccio',
        'founded': 1910,
        'city': 'Ajaccio, France',
        },

    {
        'name': 'GFC Ajaccio',
        'founded': 1910,
        'city': 'Ajaccio, France',
        },

    {
        'name': 'Troyes',
        'founded': 1986,
        'city': 'Troyes, France',
        },

    {
        'name': 'Le Mans',
        'founded': 1903,
        'city': 'Le Mans, France',
        },

    {
        'name': 'Havre',
        'founded': 1872,
        'city': 'Havre, France',
        },

    {
        'name': 'Saint-Étienne',
        'founded': 1919,
        'city': 'Saint-Étienne, France',
        },

    {
        'name': 'Sedan',
        'founded': 1919,
        'city': 'Sedan, France',
        },





    # Poland


    {
        'name': 'RKS Radomsko',
        'founded': 1979,
        'city': 'Radomsko, Poland',
        },

    {
        'name': 'Świt Nowy Dwór Mazowiecki',
        'founded': 1935,
        'city': 'Nowy Dwór Mazowiecki, Poland',
        },

    {
        'name': 'Raków Częstochowa',
        'founded': 1921,
        'city': 'Częstochowa, Poland',
        },


    {
        'name': 'Stal Mielec',
        'city': 'Mielec, Poland',
        'founded': datetime.datetime(1939, 4, 10),
        },



    {
        'name': 'Amica Wronki',
        'founded': 1992,
        'city': 'Warsaw, Poland',
        },

    {
        'name': 'Gwardia Warszawa',
        'founded': 1948,
        'city': 'Warsaw, Poland',
        },


    {
        'name': 'Arka Gdynia',
        'founded': 1929,
        'city': 'Gdynia, Poland',
        },

    {
        'name': 'Dyskobolia Grodzisk Wielkopolski',
        'founded': datetime.datetime(1922, 4, 30),
        'city': 'Grodzisk Wielkopolski, Poland',
        },

    {
        'name': 'GKS Belchatow',
        'founded': 1977,
        'city': 'Belchatow, Poland',
        },

    {
        'name': 'GKS Katowice',
        'founded': datetime.datetime(1964, 2, 27),
        'city': 'Katowice, Poland',
        },

    {
        'name': 'Szczakowianka Jaworzno',
        'founded': datetime.datetime(1923, 7, 9),
        'city': 'Jaworzno, Poland',
        },

    {
        'name': 'Pogon Szczecin',
        'founded': datetime.datetime(1948, 4, 21),
        'city': 'Szczecin, Poland',
        },

    {
        'name': 'Gornik Zabrze',
        'founded': 1948,
        'city': 'Zabrze, Poland',
        },

    {
        'name': 'Górnik Łęczna',
        'founded': 1979,
        'city': 'Łęczna, Poland',
        },

    {
        'name': 'Hutnik Nowa Huta',
        'founded': 1950,
        'city': 'Kraków, Poland',
        },


    {
        'name': 'Jagiellonia Białystok',
        'founded': datetime.datetime(1920, 5, 30),
        'city': 'Bialystok, Poland',
        },

    {
        'name': 'KS Cracovia',
        'founded': datetime.datetime(1906, 6, 13),
        'city': 'Krakow, Poland',
        },

    {
        'name': 'KS Polkowice',
        'founded': 1947,
        'city': 'Polkowice, Poland',
        },

    {
        'name': 'KS Ruch',
        'founded': datetime.datetime(1920, 4, 20),
        'city': 'Chorzów, Poland',
        },

    {
        'name': 'Korona Kielce',
        'founded': datetime.datetime(1973, 7, 10),
        'city': 'Kielce, Poland',
        },

    {
        'name': 'Legia Warszawa',
        'founded': datetime.datetime(1916, 3, 5), 
        'city': 'Warsaw, Poland',
        },

    {
        'name': 'Lechia Gdańsk',
        'founded': 1945,
        'city': 'Gdansk, Poland',
        },

    {
        'name': 'Lech Poznań',
        'founded': datetime.datetime(1922, 3, 19),
        'city': 'Poznan, Poland',
        },
    {
        'name': 'LKS Lodz',
        'founded': 1908,
        'city': 'Lodz, Poland',
        },

    {
        'name': 'Odra Wodzisław',
        'founded': 1922,
        'city': 'Wodzisław Śląski, Poland',
        },

    {
        'name': 'Ostrowiec Świętokrzyski',
        'founded': datetime.datetime(1929, 8, 11),
        'city': 'Ostrowiec Świętokrzyski, Poland',
        },


    {
        'name': 'Piast Gliwice',
        'founded': datetime.datetime(1945, 6, 18),
        'city': 'Gliwice, Poland',
        },

    {
        'name': 'Polonia Bytom',
        'founded': datetime.datetime(1920, 1, 4),
        'city': 'Bytom, Poland',
        },

    {
        'name': 'Szombierki Bytom',
        'founded': 1919,
        'city': 'Bytom, Poland',
        },

    {
        'name': 'Polonia Warsaw',
        'founded': 1911,
        'city': 'Warsaw, Poland',
        },

    {
        'name': 'Ruch Radzionków',
        'founded': 1919,
        'dissolved': 2012,
        'city': 'Radzionkow, Poland',
        },

    {
        'name': 'Podbeskidzie Bielsko-Biała',
        'founded': 1995,
        'city': 'Bielsko-Biała, Poland',
        },

    {
        'name': 'Śląsk Wrocław',
        'founded': 1947,
        'city': 'Wroclaw, Poland',
        },

    {
        'name': 'Stomil Olsztyn',
        'founded': datetime.datetime(1945, 7, 15),
        'city': 'Olsztyn, Poland',
        },

    {
        'name': 'Wisla Krakow',
        'founded': 1906,
        'city': 'Kraków, Poland',
        },

    {
        'name': 'Wisla Plock',
        'founded': 1947,
        'city': 'Plock, Poland',
        },

    {
        'name': 'Widzew Łódź',
        'founded': datetime.datetime(1910, 11, 5),
        'city': 'Lodz Poland',
        },

    {
        'name': 'Zagłębie Lubin',
        'founded': datetime.datetime(1945, 9, 10),
        'city': 'Lubin, Poland',
        },

    {
        'name': 'Zagłębie Sosnowiec',
        'founded': 1906,
        'city': 'Sosnowiec, Poland',
        },

    {
        'name': 'Garbarnia Kraków',
        'founded': 1921,
        'city': 'Krakow, Poland',
        },

    {
        'name': 'Świt Nowy Dwór',
        'founded': 1935,
        'city': 'Nowy Dwór Mazowiecki, Poland',
        },

    {
        'name': 'GKS Tychy',
        'founded': datetime.datetime(1971, 4, 20),
        'city': 'Nowy Dwór Mazowiecki, Poland',
        },


    # Scotland

    {
        'name': 'Motherwell',
        'founded': datetime.datetime(1886, 5, 17),
        'city': 'Motherwell, Scotland',
        },

    {
        'name': 'Raith Rovers',
        'founded': 1883,
        'city': 'Kirkcaldy, Scotland',
        },

    {
        'name': 'St. Mirren',
        'founded': 1877,
        'city': 'Paisley, Scotland',
        },

    {
        'name': 'Hibernian F.C.',
        'founded': 1875,
        'city': 'Leith, Scotland',
        },

    {
        'name': 'Dunfermline',
        'founded': datetime.datetime(1885, 6, 1),
        'city': 'Dunfermline, Scotland',
        },

    {
        'name': 'Dundee United',
        'founded': datetime.datetime(1909, 5, 24),
        'city': 'Dundee, Scotland',
        },

    {
        'name': 'St. Johnstone',
        'founded': 1884,
        'city': 'Perth, Scotland',
        },

    {
        'name': 'Dundee FC',
        'founded': 1893,
        'city': 'Dundee, Scotland',
        },


    {
        'name': 'Aberdeen F.C.',
        'founded': 1903,
        'city': 'Aberdeen, Scotland',
        },


    {
        'name': 'Glasgow Celtic F.C.',
        'founded': 1888,
        'city': 'Glasgow, Scotland',
        },


    {
        'name': 'Glasgow Rangers',
        'founded': 1872,
        'city': 'Glasgow, Scotland',
        },


    {
        'name': 'Heart of Midlothian',
        'founded': 1874,
        'city': 'Edinburgh, Scotland',
        },


    {
        'name': 'Kilmarnock F.C.',
        'founded': datetime.datetime(1869, 1, 5),
        'city': 'Kilmarnock, Scotland',
        },
    




    # Denmark
    {
        'name': 'AGF Aarhus',
        'founded': 1880,
        'city': 'Aarhus, Denmark',
        },

    
    {
        'name': 'Næstved BK',
        'founded': 1939,
        'city': 'Naestved, Denmark',
        },


    {
        'name': 'Viborg FF',
        'founded': 1896, 
        'city': 'Viborg, Denmark',
        },


    {
        'name': 'Ikast FS',
        'founded': 1935,
        'city': 'Ikast, Denmark',
        },


    {
        'name': 'Nørresundby Boldklub',
        'founded': datetime.datetime(1946, 6, 6),
        'city': 'Aalborg, Denmark',
        },

    {
        'name': 'AaB',
        'founded': datetime.datetime(1885, 5, 1),
        'city': 'Aalborg, Denmark',
        },

    {
        'name': 'AB',
        'founded': 1889,
        'city': 'Gladsaxe, Denmark',
        },

    {
        'name': 'OB',
        'founded': 1887,
        'city': 'Odense, Denmark',
        },

    {
        'name': 'FC Midtjylland',
        'founded': datetime.datetime(1999, 2, 2),
        'city': 'Herning, Denmark',
        },

    {
        'name': 'Vejle',
        'founded': 1891,
        'city': 'Vejle, Denmark',
        },

    {
        'name': 'Nordsjælland',
        'founded': datetime.datetime(1991, 1, 1),
        'city': 'Farum, Denmark',
        },

    {
        'name': 'Randers',
        'founded': datetime.datetime(2003, 1, 1),
        'city': 'Randers, Denmark',
        },

    {
        'name': 'Brøndby',
        'founded': datetime.datetime(1964, 12, 3),
        'city': 'Brondbyvester, Denmark',
        },

    {
        'name': 'B.93',
        'founded': 1893,
        'city': 'Osterbro, Denmark',
        },

    {
        'name': 'Esbjerg fB',
        'founded': datetime.datetime(1924, 7, 23),
        'city': 'Esbjerg, Denmark',
        },

    {
        'name': 'Horsens',
        'founded': datetime.datetime(1994, 1, 1),
        'city': 'Horsens, Denmark',
        },

    {
        'name': 'SønderjyskE',
        'founded': 1901,
        'city': 'Haderslev, Denmark',
        },

    #{
    #    'name': 'Haderslev FK',
    #    'founded': datetime.datetime(1906, 7, 14),
    #    'city': 'Haderslev, Denmark',
    #    # Parent club of 
    #    },


    {
        'name': 'Herfølge BK',
        'founded': 1921,
        'city': 'Herfølge, Denmark',
        },

    {
        'name': 'FC København',
        'founded': datetime.datetime(1992, 7, 1),
        'city': 'Copenhagen, Denmark',
        },

    {
        'name': 'Aarhus Fremad',
        'founded': 1897,
        'city': 'Aarhus, Denmark',
        },

    {
        'name': 'Frem',
        'founded': datetime.datetime(1886, 7, 17),
        'city': 'Copenhagen, Denmark',
        },

    {
        'name': 'HB Køge',
        'founded': datetime.datetime(2009, 7, 1),
        'city': 'Herfolge, Denmark',
        },

    {
        'name': 'Lyngby BK',
        'founded': 1921,
        'city': 'Lyngby, Denmark',
        },

    {
        'name': 'Silkeborg IF',
        'founded': 1917,
        'city': 'Silkeborg, Denmark',
        },











    # Iceland


    {
        'name': 'KA Akureyri',
        'city': 'Akureyri, Iceland',
        'founded': 1928,
        },




    {
        'name': 'Vikingur',
        'city': 'Reykjavik, Iceland',
        'founded': datetime.datetime(1908, 4, 21),
        },

    {
        'name': 'ÍBV Vestmannaeyjar',
        'city': 'Vestmannaeyjar, Iceland',
        'founded': 1903,
        },



    {
        'name': 'Keflavik',
        'city': 'Reykjanesbær, Iceland',
        'founded': 1929,
        },



    {
        'name': 'IA Akranes',
        'city': 'Akranes, Iceland',
        'founded': 1946,
        },

    {
        'name': 'Fram Reykjavik',
        'city': 'Reykjavik, Iceland',
        'founded': datetime.datetime(1908, 5, 1),
        },


    {
        'name': 'Fylkir',
        'founded': datetime.datetime(1967, 5, 28),
        'city': 'Reykjavik, Iceland',
        },

    {
        'name': 'UMF Tindastóll',
        'founded': 1907,
        'city': 'Sauðárkrókur, Iceland',
        },


    {
        'name': 'Valur FC',
        'founded': datetime.datetime(1911, 5, 11),
        'city': 'Reykjavik, Iceland',
        },

    {
        'name': 'KR Reykjavik',
        'founded': datetime.datetime(1899, 2, 16),
        'city': 'Reykjavik, Iceland',
        },

    {
        'name': 'IBV',
        'founded': 1903,
        'city': 'Vestmannaeyjar, Iceland',
        },



    # Ireland


    {
        'name': 'Drumcondra',
        'city': 'Dublin, Ireland',
        'founded': 1923,
        'dissolved': 1972,
        },
    {
        'name': 'Shelbourne',
        'city': 'Drumcondra, Ireland',
        'founded': 1895,
        },

    {
        'name': 'Cork Hibernians',
        'city': 'Cork, Ireland',
        'founded': 1957,
        'dissolved': 1977,
        },


    {
        'name': 'University of Dublin',
        'city': 'Dublin, Ireland',
        },

    {
        'name': 'Bohemians FC',
        'founded': 1890,
        'city': 'Dublin, Ireland',
        },

    {
        'name': 'Shamrock Rovers',
        'founded': 1901,
        'city': 'Dublin, Ireland',
        },

    {
        'name': 'Cork Celtic',
        'founded': 1959,
        'dissolved': 1979,
        'city': 'Cork, Ireland',
        },

    {
        'name': 'St. Patrick\'s Athletic',
        'founded': 1929,
        'city': 'Dublin, Ireland',
        },

    {
        'name': 'Waterford United',
        'founded': 1930,
        'city': 'Waterford, Ireland',
        },

    {
        'name': 'Limerick F.C.',
        'founded': 1937,
        'city': 'Limerick, Ireland',
        },


    {
        'name': 'Finn Harps',
        'founded': 1954,
        'city': 'Ballybofey, Ireland',
        },


    {
        'name': 'Galway United FC',
        'founded': 1937,
        'city': 'Galway, Ireland',
        },
    {
        'name': 'Sligo Rovers',
        'founded': 1928,
        'city': 'Sligo, Ireland',
        },

    {
        'name': 'Dundalk F.C.',
        'founded': datetime.datetime(1903, 9, 1),
        'city': 'Dundalk, Ireland',
        },

    # Russia
    {
        'name': 'Alania Vladikavkaz',
        'founded': 1921,
        'city': 'Vladikavkaz, Russia',
        },

    {
        'name': 'FC Ural',
        'founded': 1930,
        'city': 'Yekaterinburg, Russia',
        },


    {
        'name': 'Saturn Ramenskoye',
        'founded': 1946,
        'city': 'Ramenskoye, Russia',
        },

    {
        'name': 'FK Khimki',
        'founded': 1996,
        'city': 'Khimki, Russia',
        },

    {
        'name': 'Shinnik Yaroslavl',
        'founded': 1957,
        'city': 'Yaroslavl, Russia',
        },

    {
        'name': 'Sibir Novosibirsk',
        'founded': 1936,
        'city': 'Novosibirsk, Russia',
        },


    {
        'name': 'Volga Nizhny Novgorod',
        'founded': 1963,
        'city': 'Nizhny Novgorod, Russia',
        },

    {
        'name': 'FC Rostov',
        'founded': 1930,
        'city': 'Rostov-on-Don, Russia',
        },

    {
        'name': 'Spartak Nalchik',
        'founded': 1935,
        'city': 'Nalchik, Russia',
        },

    {
        'name': 'Terek Grozny',
        'founded': 1958,
        'city': 'Grozny, Russia',
        },

    {
        'name': 'Tom Tomsk',
        'founded': 1957,
        'city': 'Tomsk, Russia',
        },

    {
        'name': 'Amkar Perm',
        'founded': 1994,
        'city': 'Perm, Russia',
        },

    {
        'name': 'FC Krasnodar',
        'founded': datetime.datetime(2008, 2, 22),
        'city': 'Krasnodar, Russia',
        },

    {
        'name': 'Kuban Krasnodar',
        'founded': 1928,
        'city': 'Krasnodar, Russia',
        },

    {
        'name': 'Anzhi Makhachkala',
        'founded': 1991,
        'city': 'Makhachkala, Russia',
        },

    {
        'name': 'CSKA Moscow',
        'founded': datetime.datetime(1911, 8, 27),
        'city': 'Moscow, Russia',
        },

    {
        'name': 'FC Dynamo Moscow',
        'founded': 1923,
        'city': 'Moscow, Russia',
        },

    {
        'name': 'Mordovia Saransk',
        'founded': 1961,
        'city': 'Saransk, Russia',
        },

    {
        'name': 'Lokomotiv Moscow',
        'founded': datetime.datetime(1936, 1, 12),
        'city': 'Moscow, Russia',
        },

    {
        'name': 'Luch-Energiya',
        'founded': 1958,
        'city': 'Vladivostok, Russia',
        },

    {
        'name': 'Rubin Kazan',
        'founded': datetime.datetime(1958, 4, 20),
        'city': 'Kazan, Russia',
        },

    {
        'name': 'Spartak Moscow',
        'founded': datetime.datetime(1922, 4, 18),
        'city': 'Moscow, Russia',
        },

    {
        'name': 'Torpedo Moscow',
        'founded': 1930,
        'city': 'Moscow, Russia',
        },


    {
        'name': 'FC Moscow',
        'founded': 2004,
        'dissolved': 2010,
        'city': 'Moscow, Russia',
        },

    {
        'name': 'FC Torpedo-ZIL Moscow',
        'founded': 1997,
        'dissolved': 2004,
        'city': 'Moscow, Russia',
        },

    {
        'name': 'Krylia Sovetov',
        'founded': 1942,
        'city': 'Samara, Russia',
        },

    {
        'name': 'FC Ufa',
        'founded': datetime.datetime(2010, 12, 23),
        'city': 'Ufa, Russia',
        },


    {
        'name': 'Zenit Saint Petersburg',
        'founded': 1924,
        'city': 'Saint Petersburg, Russia',
        },


    # Finland

    {
        'name': 'FinnPa',
        'founded': 1965,
        'dissolved': 1998,
        'city': 'Helsinki, Finland',
        },

    {
        'name': 'Ilves',
        'city': 'Tampere, Finland',
        },



    ]
